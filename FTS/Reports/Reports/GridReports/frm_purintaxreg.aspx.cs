...
namespace Reports.Reports.GridReports
{
    public partial class frm_purintaxreg : ERP.OMS.ViewState_class.VSPage
    {
       ...
       if(Session["SI_ComponentData_Branch"] != null)
              ...
              if (lookup_branch.GridView.GetSelectedFieldValues("branch_id").Count() != ((DataTable)Session["SI_ComponentData_Branch"]).Rows.Count)
                       {
                        ...
                        Task PopulateStockTrialDataTask = new Task(() => GetINTREGdata(FROMDATE, TODATE, BRANCH_ID, DOCTYPE_ID, GSTINID, custinternlid, checkpartyinvdate));
        ...
        public void GetINTREGdata(string FROMDATE, string TODATE, string BRANCH_ID, string DOCTYPE_ID, string GSTINID, string custinternlid, int checkpartyinvdate)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@VENDORID", custinternlid);
        ...
        public void bindexport(int Filter)
        {
            if (OPTREGGrid.VisibleRowCount > 0)
            {
                ...
                FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true) + Environment.NewLine + "GST Input Tax Register" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
            ...
            if (e.Filter != "")
            {
                ...
                CustomerDataSource.SelectCommand =
                       //@"select cnt_internalid,uniquename,Name,Billing from (SELECT cnt_internalid,uniquename,Name,Billing, row_number()over(order by t.[Name]) as [rn]  from v_pos_customerDetails  as t where (([uniquename] + ' ' + [Name] ) LIKE @filter)) as st where st.[rn] between @startIndex and @endIndex";
                         @"select cnt_internalid,uniquename,Name from (select cnt_id,cnt_internalid,uniquename,Name, row_number()over(order by t.[Name]) as [rn]  from  (select TMC.cnt_id,TMC.cnt_internalid,TMC.cnt_UCC as uniquename, isnull(TMC.cnt_firstName,'')+isnull(TMC.cnt_middleName,'')+isnull(TMC.cnt_lastName,'') as Name  from tbl_master_contact TMC  where cnt_contactType='DV' ) as t where (([uniquename] + ' ' + [Name] ) LIKE @filter)) tt where tt.[rn] between @startIndex and @endIndex";
