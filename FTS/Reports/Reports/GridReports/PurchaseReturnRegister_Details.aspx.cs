...
{
    public partial class PurchaseReturnRegister_Details : System.Web.UI.Page
    {
        ...
        {
            string filename = "Purchase Return Register Details Report";
            exporter.FileName = filename;
            string FileHeader = "";

            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();

            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true) + Environment.NewLine + "Purchase Return Register Details Report" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        protected void Grid_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            string TRAN_TYPE = "PR";
            Task PopulateStockTrialDataTask = new Task(() => GetPurchaseRegisterdata(FROMDATE, TODATE, BRANCH_ID, HEAD_BRANCH, TRAN_TYPE));
            PopulateStockTrialDataTask.RunSynchronously();
        }
        public void GetPurchaseRegisterdata(string FROMDATE, string TODATE, string BRANCH_ID, string HEAD_BRANCH, string TRAN_TYPE)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                SqlCommand cmd = new SqlCommand("prc_PurchaseRegisterProductsDetails_Report", con);
               ...
               // cmd.Parameters.AddWithValue("@Partydate", chkparty.Checked);
                cmd.Parameters.AddWithValue("@Partydate", 0);
