...
{
    public partial class DebtorsDetailsCR : System.Web.UI.Page
    {
        ...
        {
            string filename = "Debtor Details Credit";
            ...
            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true) + Environment.NewLine + "Debtor Details Credit" + Environment.NewLine + "As on " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        //        ComponentTable = oDBEngine.GetDataTable("select branch_id as ID,branch_description,branch_code from tbl_master_branch where UPPER(substring(branch_code,3,2)) = 'RO' order by branch_description asc");
        //    }
        ...
        //{
        //    DataTable ComponentTable = oDBEngine.GetDataTable("SELECT DISTINCT A.branch_id,A.branch_description as Code FROM tbl_master_branch A, tbl_master_branch B WHERE A.branch_id=B.branch_parentId ORDER BY A.branch_id asc");
