...
{
    public partial class GSTR_Customerpaymentreceipt : System.Web.UI.Page
    {
        ...
        {
            string filename = "GSTR customer payment Receipt";
            exporter.FileName = filename;
            string FileHeader = "";

            exporter.FileName = filename;
            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();
            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true) + Environment.NewLine + "GSTR customer payment Receipt" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        protected void ShowGrid_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            ...
            //decimal taxableamt = 0, adjusted = 0, cgst = 0, sgst = 0, utgst = 0, igst = 0, balanceamt = 0;

            //if (e.Item.FieldName == "Voucher_Amount")
            //{
            //    taxableamt = Convert.ToDecimal(e.Value);
            //}
            //else if (e.Item.FieldName == "Adjusted_Tax_Amount")
            //{
            //    adjusted = Convert.ToDecimal(e.Value);
            //}

            //if (e.Item.FieldName == "Adjusted_CGST")
            //{
            //    cgst = Convert.ToDecimal(e.Value);
            //}

            //if (e.Item.FieldName == "Adjusted_SGST")
            //{
            //    sgst = Convert.ToDecimal(e.Value);
            //}
            //else if (e.Item.FieldName == "Adjusted_UTGST")
            //{
            //    utgst = Convert.ToDecimal(e.Value);
            //}

            //if (e.Item.FieldName == "Adjusted_IGST")
            //{
            //    igst = Convert.ToDecimal(e.Value);
            //}

            //if (e.Item.FieldName == "Balanced_Amount")
            //{
            //    balanceamt = Convert.ToDecimal(e.Value);
            //}

            //if (e.Item.FieldName == "Balanced_Amount")
            //{
            //    e.Text = Convert.ToString(Convert.ToDecimal(taxableamt - (adjusted + cgst + sgst+utgst+igst)));
            //}
            //else
            //{
        ...
        protected void ASPxGridView1_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
        {
            ...
            if (e.SummaryProcess == DevExpress.Data.CustomSummaryProcess.Calculate)
            {
                ...
                if (!Invoice_Number.Contains(val))
                {
                    totalCount++;
                    Invoice_Number.Add(val);
                }
            ...
            if (e.SummaryProcess == DevExpress.Data.CustomSummaryProcess.Finalize)
            {
                ...
                if (e.Item == ShowGrid.TotalSummary["Invoice_Number"])
                {
                ...
                {
                    e.TotalValue = string.Format("Doc. Count={0}", totalCountrecp);
