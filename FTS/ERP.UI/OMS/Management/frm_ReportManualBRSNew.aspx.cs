...
namespace ERP.OMS.Management
{
    ...
    public partial class Reports_frm_ReportManualBRSNew : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
    {
        ...
        void Procedure()
        {
            ...
            {
                CompanyId = "'" + Convert.ToString(Session["LastCompany"].ToString().Trim()) + "'";
            }
            else
            {
            ...
            ds = oManagement_BL.Report_BRSNew(Convert.ToString(DdlReportView.SelectedItem.Value), CompanyId, BanckAc, Convert.ToString(DdlConsider.SelectedItem.Value.ToString()), Convert.ToDateTime(DtAsOn.Value).ToString("yyyy-MM-dd"),
                Convert.ToString(Session["LastFinYear"].ToString()));
            ViewState["dataset"] = ds;
            FunctionCall(ds);
