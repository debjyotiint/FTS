...
namespace ERP.OMS.Management.DailyTask
{
    ...
    public partial class management_DailyTask_frm_attendance_employeeWise : ERP.OMS.ViewState_class.VSPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                cmbFromdate.EditFormatString = Oconverter.GetDateFormat("Date");
                ...
                //set from value current date
                cmbFromdate.Value = oDBEngine.GetDate();
                //end set from value current date
        ...
        private void BindGrid()
        {
            ...
            if (txtName_hidden.Text != "")
            {
                ...
                DataTable CheckLeaveDate = oDBEngine.GetDataTable("tbl_master_employee", "emp_dateofleaving", "emp_contactid='" + txtName_hidden.Text.ToString() + "'");
                if (CheckLeaveDate.Rows.Count > 0)
                {
                    if (CheckLeaveDate.Rows[0][0].ToString() != "1/1/1900 12:00:00 AM" && CheckLeaveDate.Rows[0][0].ToString() != "")
                    {
                        int Val = DateTime.Compare(DateTime.Parse(cmbDate.Value.ToString()), DateTime.Parse(CheckLeaveDate.Rows[0][0].ToString()));
                        if (Val > 0)
                        {
                            Todate = DateTime.Parse(cmbDate.Value.ToString());
                            cmbDate.Value = DateTime.Parse(CheckLeaveDate.Rows[0][0].ToString());
                            TrLeaveDate.Visible = true;
                            string mm, dd, yy;
                            string[] sp = CheckLeaveDate.Rows[0][0].ToString().Split('/');
                            mm = sp[0].ToString();
                            dd = sp[1].ToString();
                            yy = sp[2].ToString();
                            lblLEaveDate.Text = dd + "-" + mm + "-" + yy;

                        }
                    }
                    else
                    {
                        TrLeaveDate.Visible = false;
                    }
                }
                else
                {
                        ...
                        for (int k = DateTime.Parse(cmbFromdate.Value.ToString()).Day; k <= DateTime.Parse(cmbDate.Value.ToString()).Day; k++)
                        {
                            if (Query == "")
                                Query = "select convert(varchar(12),cast('" + j.ToString() + "/" + k.ToString() + "/" + i.ToString() + "' as datetime),113) as date, *,(select atd_intime from tbl_trans_AttendanceDetail where atd_atdid=cast(D.atd_id as numeric) and convert(varchar(11),atd_dateofattendance,113)=convert(varchar(11),cast('" + j.ToString() + "/" + k.ToString() + "/" + i.ToString() + "' as datetime),113)) as intime," +
                                        ...
                                        " from (select atd_id,ISNULL(atd_statusday" + k + ",'P') as status from tbl_trans_attendance where atd_cntid='" + txtName_hidden.Text.Trim() + "' and atd_month=month('" + j.ToString() + "/" + k.ToString() + "/" + i.ToString() + "') and atd_year=year('" + j.ToString() + "/" + k.ToString() + "/" + i.ToString() + "')) as D ";
                            else
                ...
                if (attdnce.Length <= 0 || attdnce[0][fieldName].ToString() == "")
                {
                    ...
                    if (Leave.Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            expression = " holw_workId=" + data_WorkingHourEmployee[0]["workingID"].ToString().Trim() + " and hol_DateOfHoliday='" + date.Text + "'";
