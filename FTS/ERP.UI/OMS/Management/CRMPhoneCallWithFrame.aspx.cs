    ...
    public partial class management_CRMPhoneCallWithFrame : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        protected void btnSavePhoneCallDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                flag = Convert.ToBoolean(oDBEngine.InsurtFieldValue("tbl_trans_phonecalldetails", "phd_phoneCallId,phd_branchId,phd_callDate,phd_callStart,phd_callEnd,phd_callDispose,phd_note,phd_nextCall,phd_callduration,CreateDate,CreateUser", "'" + phonecallid + "','" + branchid + "','" + calldate + "','" + CallStart + "','" + call + "','" + calldispose + "','" + txtNotes.Text + "','" + str + "','" + Callduration + "','" + oDBEngine.GetDate().ToString("yyyy-MM-dd HH:mm:ss") + "','" + Convert.ToString(Session["userid"]) + "'"));
                if (flag)
                {
