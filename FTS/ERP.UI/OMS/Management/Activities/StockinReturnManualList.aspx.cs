...
{
    public partial class StockinReturnManualList : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/StockinReturnManualList.aspx");

            if (!IsPostBack)
            {
                // Session.Remove("StockInReturnManualListingDetails");
                if (Session["LastCompany"] != null && Session["LastCompany"] != null)
                {
                    ...
                    GrdSalesReturn.SettingsCookies.CookiesID = "BreeezeErpGridCookiesSalesStockinReturnManualListDetailsGrid";
                    this.Page.ClientScript.RegisterStartupScript(GetType(), "heightL", "<script>addCookiesKeyOnStorage('BreeezeErpGridCookiesSalesStockinReturnManualListDetailsGrid');</script>");
        ...
        public void bindexport(int Filter)
        {
            ...
            GrdSalesReturn.Columns[16].Visible = false;
            string filename = "Receive Against Manual Return";
            exporter.FileName = filename;
            //  exporter.FileName = "GrdSalesStockinReturnManual";
            exporter.Landscape = true;
            exporter.PaperKind = System.Drawing.Printing.PaperKind.A4;
            exporter.MaxColumnWidth = 200;
            exporter.PageHeader.Left = "Receive Against Manual Return";
        ...
        protected void GrdSalesReturn_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Command == "Delete")
            {
                ...
                int deletecnt = 0;
                deletecnt = objSalesReturnBL.DeleteSalesReturn(SalesReturnID, Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), "SISRM", Convert.ToString(Session["userbranchID"]));
                ...
                dtdata = objSalesReturnBL.GetCustomerStockInReturnListGridData(Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]), Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), FromDate, ToDate, "SRM", ubranch, BranchID);

                if (dtdata != null && dtdata.Rows.Count > 0)
                {
                    // Session["StockInReturnManualListingDetails"] = dtdata;
            ...
            else if (Command == "FilterGridByDate")
            {
                ...
                dtdata = objSalesReturnBL.GetCustomerStockInReturnListGridData(userbranch, lastCompany, finyear, FromDate, ToDate, "SRM", ubranch, BranchID);
                //  Session["StockInReturnManualListingDetails"] = dtdata;
                if (dtdata != null && dtdata.Rows.Count > 0)
                {
        ...
        public void GetSalesReturnListGridData(string userbranch, string lastCompany, string FinyearStartDate, string FinYearEndDate)
        {
            ...
            //string ubranch = Convert.ToString(HttpContext.Current.Session["UserSiblingBranchHierarchy"]);
            //DataTable dtdata = new DataTable();
            //dtdata = objSalesReturnBL.GetCustomerStockInReturnListGridData(userbranch, lastCompany, FinyearStartDate, FinYearEndDate, "SRM", ubranch, finyear);
            //if (dtdata != null && dtdata.Rows.Count > 0)
            //{
            //    GrdSalesReturn.DataSource = dtdata;
            //    GrdSalesReturn.DataBind();
        ...
        //DataTable dtdata = new DataTable();
        //dtdata = objSalesReturnBL.GetCustomerStockInReturnListGridData(userbranch, lastCompany, FinyearStartDate, FinYearEndDate, "SRM", userbranch, finyear);
        //if (dtdata != null && dtdata.Rows.Count > 0)
        //{
        //    GrdSalesReturn.DataSource = dtdata;
        ...
        //  DataTable dsdata = (DataTable)Session["StockInReturnManualListingDetails"];
        ...
        protected void EntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_StockInReturnManualLists
