...
{
    public partial class OldUnit_SalesInvoice : ERP.OMS.ViewState_class.VSPage
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/OldUnit_SalesInvoice.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["key"] != null)
                {
                    ...
                    {
                        lblHeadTitle.Text = "Modify Second Hand Sales";
                        ...
                        DataTable Productdt = objSalesInvoiceBL.GetOldUnitInvoiceProductData(strQuotationId).Tables[0];
                        ...
                        {
                            lblHeadTitle.Text = "View Second Hand Sales";
                    ...
                    else
                    {
                        ...
                        Session["SI_TaxDetails"] = null;
                        CreateDataTaxTable();
                        lblHeadTitle.Text = "Add Second Hand Sales";
        ...
        public class Quotation
        {
            ...
            public string ProdAvgvalue { get; set; }
        }
        #endregion
        ...
        public IEnumerable GetQuotation(DataTable Quotationdt)
        {
            ...
            if (Quotationdt != null && Quotationdt.Rows.Count > 0)
            {
                for (int i = 0; i < Quotationdt.Rows.Count; i++)
                {
                    ...
                    Quotations.ProdAvgvalue  = Convert.ToString(Quotationdt.Rows[i]["ProdAvgvalue"]);
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            else
            {
                ...
                Quotationdt.Columns.Add("ProdAvgvalue", typeof(string));
            }
            ...
            foreach (var args in e.InsertValues)
            {
                ...
                if (ProductDetails != "" && ProductDetails != "0")
                {
                    ...
                    string ProdAvgvalue = (Convert.ToString(args.NewValues["ProdAvgvalue"]) != "") ? Convert.ToString(args.NewValues["ProdAvgvalue"]) : "0";

                    Quotationdt.Rows.Add(SrlNo, "0", ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM, SalePrice, Discount, Amount, TaxAmount, TotalAmount, "I", ProductName, ComponentID, ComponentName, TotalQty, BalanceQty, "", IsLinkedProduct, ProdAvgvalue);
            ...
            foreach (var args in e.UpdateValues)
            {
                ...
                if (isDeleted == false)
                {
                    if (ProductDetails != "" && ProductDetails != "0")
                    {
                        ...
                        string IsComponentProduct = (Convert.ToString(args.NewValues["IsComponentProduct"]) != "") ? Convert.ToString(args.NewValues["IsComponentProduct"]) : "N";
                        string IsLinkedProduct = (Convert.ToString(args.NewValues["IsLinkedProduct"]) != "") ? Convert.ToString(args.NewValues["IsLinkedProduct"]) : "N";
                        string ProdAvgvalue = (Convert.ToString(args.NewValues["ProdAvgvalue"]) != "") ? Convert.ToString(args.NewValues["ProdAvgvalue"]) : "N";
                        ...
                        if (Isexists == false)
                        {
                            Quotationdt.Rows.Add(SrlNo, QuotationID, ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM, SalePrice, Discount, Amount, TaxAmount, TotalAmount, "U", ProductName, ComponentID, ComponentName, TotalQty, BalanceQty, IsComponentProduct, IsLinkedProduct, ProdAvgvalue);
                        }
            ...
            if (IsDeleteFrom != "D" && IsDeleteFrom != "C")
            {
               ...
               .Select(g => g.Key);
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" || validate == "duplicateProduct" || validate == "nullAmount" || validate == "nullQuantity" || validate == "transporteMandatory" || validate == "TCMandatory" || validate == "minSalePriceMust"   || validate == "DueDateLess")
                {
                ...
                else
                {
                    ...
                    TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialForOldUnit(ref tempQuotation, "SrlNo", "ProductID", "Amount", "TaxAmount", TaxDetailTable, "S", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, TaxType);
                    ...
                    ModifyQuatation(strIsInventory, MainInvoiceID, strSchemeType, UniqueQuotation, strInvoiceDate, strCustomer, strContactName,
                                    ...
                                    approveStatus, ActionType, ref strIsComplete, ref strQuoteID, paymentDetails);
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                Session["SI_QuotationDetails"] = dt_QuotationDetails;
        ...
        public void ModifyQuatation(string strIsInventory, string QuotationID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strCustomer, string strContactName,
                                    ...
                                    ref int strIsComplete, ref int strInvoiceID, DataTable paymentDetails)
        {
            try
            {
                // ASPxTextBox txtDriverName = (ASPxTextBox)VehicleDetailsControl.FindControl("txtDriverName");
                ...
                SqlCommand cmd = new SqlCommand("prc_OldUnitCRMSalesInvoice", con);
        ...
        public void GetAllDropDownDetailForSalesQuotation(string userbranch)
        {
            ...
            SlaesActivitiesBL objSlaesActivitiesBL = new SlaesActivitiesBL();
            DataTable Schemadt = objSlaesActivitiesBL.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "52", "Y");
        ...
        protected void ComponentQuotation_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            else if (e.Parameter.Split('~')[0] == "BindComponentGridOnSelection")//Subhabrata for binding quotation
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    ComponentQuotationPanel.JSProperties["cpDetails"] = Reference + "~" + Currency_Id + "~" + SalesmanId + "~" + ExpiryDate + "~" + CurrencyRate + "~" + Type + "~" + CreditDays + "~" + DueDate;
                }
            }
        ...
        public void BindProductLookup() 
        {
            DataTable ds = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("Prc_OldSale_det");
            proc.AddVarcharPara("@Action", 500, "AllProductDetails");
            proc.AddVarcharPara("@TransDate", 10, dt_PLQuote.Date.ToString("yyyy-MM-dd"));
            ds = proc.GetTable();
            Session["productLookUp_dsoldunit"] = ds;
            productLookUp.DataBind();
        }

        protected void productLookUp_DataBinding(object sender, EventArgs e)
        {
            if (Session["productLookUp_dsoldunit"] != null)
            {
                productLookUp.DataSource = (DataTable)Session["productLookUp_dsoldunit"];
            }
        }
