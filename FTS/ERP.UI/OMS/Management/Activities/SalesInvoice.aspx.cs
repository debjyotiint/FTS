...
namespace ERP.OMS.Management.Activities
{
    public partial class SalesInvoice : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        protected void lsmdsProduct_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            e.KeyExpression = "Products_ID";
            string connectionString = ConfigurationManager.ConnectionStrings["crmConnectionString"].ConnectionString;
            string strProductType = Convert.ToString(ProductType.Value);

            if(strProductType=="N")
            {
                ERPDataClassesDataContext dc = new ERPDataClassesDataContext(connectionString);
                var q = from d in dc.v_Product_MargeDetails
                        where d.IsInventory=="No" && d.IsCapitalGoods==false
            ...
            else if (strProductType == "C")
            {
                ERPDataClassesDataContext dc = new ERPDataClassesDataContext(connectionString);
                var q = from d in dc.v_Product_MargeDetails
                        where d.IsInventory == "No" && d.IsCapitalGoods==true
                        orderby d.sProductsID descending
                        select d;
                e.QueryableSource = q;
            }
