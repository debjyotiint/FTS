    ...
    public partial class ReturnNormalList : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        protected void GrdSalesReturn_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Command == "Delete")
            {
                ...
                int deletecnt = 0;
                deletecnt = objSalesReturnBL.DeleteSalesReturn(SalesReturnID, Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), "SRN", Convert.ToString(Session["userbranchID"]));
                if (deletecnt == 1)
                {
