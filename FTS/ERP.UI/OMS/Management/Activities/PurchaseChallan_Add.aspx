<%@ Page Title="Purchase Challan" Language="C#" MasterPageFile="~/OMS/MasterPage/ERP.Master" AutoEventWireup="true" EnableEventValidation="false" CodeBehind="PurchaseChallan_Add.aspx.cs" Inherits="ERP.OMS.Management.Activities.PurchaseChallan_Add" %>
...
function ShowVendor(){
    ShouldCheck="Y";
    cVendorComboBox.ShowDropDown();
}

function GetVendorAddress() {
    if(cVendorComboBox.GetText!="")
    {
        if (!cVendorComboBox.FindItemByValue(cVendorComboBox.GetValue())) {
            jAlert("Vendor does not Exist.", "Alert", function () { cVendorComboBox.SetValue(); cVendorComboBox.Focus(); });
            return;
        }
    }

    var VendorID = cVendorComboBox.GetValue();
    ShouldCheck="N";
...
function taggingListButnClick(s, e) {
    ...
    if (VendorID != null) {
        ...
        cQuotationComponentPanel.PerformCallback('BindComponentGrid' + '~' + VendorID + '~' + startDate + '~' + strBranch);
        cpopup_Tagging.Show();
    }
}
...
function CloseGridQuotationLookup() {
    clookupOrder.ConfirmCurrentSelection();
    clookupOrder.HideDropDown();
    clookupOrder.Focus();
}

function QuotationNumberChanged() {
    var quote_Id = clookupOrder.gridView.GetSelectedKeysOnPage(); //clookupOrder.GetValue();
    ...
    if (quote_Id != null) {
        ...
        if (arr.length > 1) {
            ...
            if (arr.length == 1) {
                ...
                var selectIndex = clookupOrder.gridView.GetFocusedRowIndex()
                var orderDate = clookupOrder.gridView.GetRow(selectIndex).children[2].innerText;
...
function PerformCallToGridBind() {
    ...
    var quote_Id = clookupOrder.gridView.GetSelectedKeysOnPage();
...
function ddl_Currency_Rate_Change() {
    ...
    <script>
        ...
        function ProductButnClick(s, e) {
            if (e.buttonIndex == 0) {
                ...
                if(VendorID!=null){
                    ...
                    jAlert("Please Select a Vendor", "Alert", function () { cVendorComboBox.Focus(); });
                }
            }
        }
        ...
        function GetProductDetails() {            
            if (!cmbProductList.FindItemByValue(cmbProductList.GetValue())) {
                jAlert("Product does not Exist.", "Alert", function () { cmbProductList.SetValue(); cmbProductList.Focus(); });
                return;
            }

            cVendorComboBox.SetEnabled(false);
            cddl_AmountAre.SetEnabled(false);            
            document.getElementById("ddl_numberingScheme").disabled = true;
            document.getElementById("ddlInventory").disabled = true;

            var ProductID = cmbProductList.GetValue();
            if (ProductID == "") {
                cProductpopUp.Hide();
            ...
            var Product_ID = cmbProductList.GetValue();
            var Product_Name = cmbProductList.GetSelectedItem().texts[0];
            var HSNCode = cmbProductList.GetSelectedItem().texts[2];
            var Product_Code = cmbProductList.GetSelectedItem().texts[5];
            var Purchase_UOMID = cmbProductList.GetSelectedItem().texts[6];
            var Purchase_UOM = cmbProductList.GetSelectedItem().texts[7];
            var Purchase_Price = cmbProductList.GetSelectedItem().texts[8];
            var IsPackingActive = cmbProductList.GetSelectedItem().texts[9];
            var Packing_Factor = cmbProductList.GetSelectedItem().texts[10];
            var Packing_UOM = cmbProductList.GetSelectedItem().texts[11];
            var Warehousetype = cmbProductList.GetSelectedItem().texts[12];
            var IsComponent = cmbProductList.GetSelectedItem().texts[13];
            var ComponentProduct = cmbProductList.GetSelectedItem().texts[14];

            if (ListOfProduct.filter(function (e) { return e.Product_ID == Product_ID; }).length == 0) {
                var Products = {
                    Product_ID: Product_ID, Product_Name: Product_Name, Product_Code: Product_Code,
                    Purchase_UOMID: Purchase_UOMID, Purchase_UOM: Purchase_UOM, Purchase_Price: Purchase_Price,
                    IsPackingActive: IsPackingActive, Packing_Factor: Packing_Factor, Packing_UOM: Packing_UOM,
                    Warehousetype: Warehousetype, IsComponent: IsComponent, ComponentProduct: ComponentProduct, 
                    HSNCode: HSNCode
                }
                ListOfProduct.push(Products);

                var JsonProductList = JSON.stringify(ListOfProduct);
                GetObjectID('hfProduct_Json').value = JsonProductList;
            }
                ...
                if (refreshType == "E") {
                    ...
                    if (PurchaseOrder_Number != "") {
                        jAlert(Order_Msg, 'Alert Dialog: [PurchaseOrder]', function (r) {
                            ...
                            grid.cpSalesOrderNo = null;
                            if (r == true) {
                                window.location.assign("PurchaseChallanList.aspx?key=ADD");
                            }
                        });
                    }
                    else {
...
function OnCustomButtonClick(s, e) {
    if (e.buttonID == 'CustomAddNewRow') {
        ...
        var IsComponentProduct=ListOfProduct.find(o => o.Product_ID === ProductID).IsComponent;
                
        if (IsComponentProduct == "Y") {
            var ComponentProduct=ListOfProduct.find(o => o.Product_ID === ProductID).ComponentProduct;
        ...
        if (noofvisiblerows != "1" && clookupOrder.GetValue() == null) {
        ...
        if (inventoryType == "C" || inventoryType == "Y" || inventoryType == "B") {
            ...
            if (QuantityValue == "0.0") {
                ...
                if (ProductID != "") {
                    var Product_Name=ListOfProduct.find(o => o.Product_ID === ProductID).Product_Name;
                    var Purchase_UOM=ListOfProduct.find(o => o.Product_ID === ProductID).Purchase_UOM;
                    var Warehousetype=ListOfProduct.find(o => o.Product_ID === ProductID).Warehousetype;
...
function DiscountTextChange(s, e) {
    ...
    if (ProductID != null) {
        var Amount = parseFloat(Quantity) * (parseFloat(PurchasePrice) / parseFloat(strRate));
        ...
        var HSNCode=ListOfProduct.find(o => o.Product_ID === ProductID).HSNCode;
        var TaxType = "";
...
function OnAddNewClick() {
            ...
            if (e.buttonIndex == 0) {
                if (cddl_AmountAre.GetValue() != null) {
                    ...
                    if (ProductID.trim() != "") {
                        ...
                        var StockQuantity = QuantityValue;
                        var Amount = parseFloat(QuantityValue * (strSalePrice / strRate)).toFixed(2);
                        ...
                        var PageStatus = GetObjectID('hdnPageStatus').value;
                        if (PageStatus=="EDIT") {
                            if (globalRowIndex > -1) {
    ...
    <script>
        ...
        function SaveNew_Click() {
            ...
            var customerId = cVendorComboBox.GetValue();
    ...
    <script>
        ...
        function SetCustomer(Id, Name) {
            if (Id != "") {
                LoadCustomerAddress(Id, $('#ddl_Branch').val(), 'PC');
                page.tabs[0].SetEnabled(true);
                page.tabs[1].SetEnabled(true);
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                    ...
                    <ContentCollection>
                        <dxe:ContentControl runat="server">
                            <div class="row">
                                ...
                                <div class="col-md-2 lblmTop8">
                                    <dxe:ASPxLabel ID="lbl_Customer" runat="server" Text="Vendor (3 Char)">
                                    </dxe:ASPxLabel>
                                    <span style="color: red;">*</span>
                                    <dxe:ASPxComboBox ID="VendorComboBox" Visible="false" runat="server" EnableCallbackMode="true" CallbackPageSize="15"
                                        ValueType="System.String" ValueField="cnt_internalid" ClientInstanceName="cVendorComboBox" Width="100%"
                                        TextFormatString="{0}" DropDownStyle="DropDown" FilterMinLength="3"
                                        OnItemsRequestedByFilterCondition="VendorComboBox_ItemsRequestedByFilterCondition"
                                        OnItemRequestedByValue="VendorComboBox_ItemRequestedByValue">
                                        <Columns>
                                            <dxe:ListBoxColumn FieldName="Name" Caption="Name" Width="400px" />
                                        </Columns>
                                        <ClientSideEvents ValueChanged="function(s, e) {GetVendorAddress()}" />
                                    </dxe:ASPxComboBox>
                                    ...
                                    <%--GotFocus="function(s,e){ShowVendor()}"--%>
                                </div>
                                <div class="clear"></div>
                                <div class="col-md-2 lblmTop8">
                                ...
                                <div class="row">
                                    <dxe:ASPxGridView ID="grid" ClientInstanceName="grid" runat="server" KeyFieldName="SrlNo"
                                        ...
                                        <Columns>
                                            ...
                                            <dxe:GridViewDataSpinEditColumn FieldName="Discount" Caption="Disc(%)" VisibleIndex="9" Width="4%" HeaderStyle-HorizontalAlign="Right">
                                                <PropertiesSpinEdit MinValue="0" MaxValue="100" AllowMouseWheel="false" DisplayFormatString="0.00" MaxLength="6">
                                                    <ClientSideEvents GotFocus="DiscountTextFocus" LostFocus="DiscountTextChange" />
                                            ...
                                            <dxe:GridViewDataButtonEditColumn FieldName="TaxAmount" Caption="Charges" VisibleIndex="11" Width="9%" HeaderStyle-HorizontalAlign="Right" ReadOnly="true">
                                            ...
                                            <dxe:GridViewDataTextColumn FieldName="NetAmount" Caption="Net Amount" VisibleIndex="12" Width="9%" HeaderStyle-HorizontalAlign="Right" ReadOnly="true">
                                                <PropertiesTextEdit DisplayFormatString="0.00"></PropertiesTextEdit>
                                            ...
                                            <dxe:GridViewDataTextColumn FieldName="ProductID" Caption="ProductID" VisibleIndex="19" ReadOnly="True" Width="0">
                                            </dxe:GridViewDataTextColumn>
                                            <dxe:GridViewDataTextColumn FieldName="SrlNo" Caption="SrlNo" VisibleIndex="20" ReadOnly="True" Width="0">
                                            </dxe:GridViewDataTextColumn>
                                        </Columns>
    ...
    <div>
        <dxe:ASPxPopupControl ID="ProductpopUp" runat="server" ClientInstanceName="cProductpopUp"
            CloseAction="CloseButton" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter" Height="300px"
            Width="900px" HeaderText="Select Product" AllowResize="true" ResizingMode="Postponed" Modal="true">
            <HeaderTemplate>
            ...
            <ContentCollection>
                <dxe:PopupControlContentControl runat="server">
                    ...
                    <dxe:ASPxComboBox ID="cmbProductList" runat="server" EnableCallbackMode="true" CallbackPageSize="15"
                        ValueType="System.String" ValueField="Product_ID" ClientInstanceName="cmbProductList" Width="100%"
                        TextFormatString="{0}" DropDownStyle="DropDown" FilterMinLength="4"
                        OnItemsRequestedByFilterCondition="cmbProductList_ItemsRequestedByFilterCondition"
                        OnItemRequestedByValue="cmbProductList_ItemRequestedByValue">
                        <Columns>
                            <dxe:ListBoxColumn FieldName="Product_Name" Caption="Name" Width="360px" />
                            <dxe:ListBoxColumn FieldName="IsInventory" Caption="IsInventory" Width="60px" />
                            <dxe:ListBoxColumn FieldName="HSNSAC" Caption="HSNSAC" Width="80px" />
                            <dxe:ListBoxColumn FieldName="ClassCode" Caption="Class" Width="230px" />
                            <dxe:ListBoxColumn FieldName="BrandName" Caption="Brand" Width="130px" />
                            <dxe:ListBoxColumn FieldName="Product_Code" Caption="Product_Code" Width="0" />
                            <dxe:ListBoxColumn FieldName="Purchase_UOMID" Caption="Purchase_UOMID" Width="0" />
                            <dxe:ListBoxColumn FieldName="Purchase_UOM" Caption="Purchase_UOM" Width="0" />
                            <dxe:ListBoxColumn FieldName="Purchase_Price" Caption="Purchase_Price" Width="0" />
                            <dxe:ListBoxColumn FieldName="IsPackingActive" Caption="IsPackingActive" Width="0" />
                            <dxe:ListBoxColumn FieldName="Packing_Factor" Caption="Packing_Factor" Width="0" />
                            <dxe:ListBoxColumn FieldName="Packing_UOM" Caption="Packing_UOM" Width="0" />
                            <dxe:ListBoxColumn FieldName="Warehousetype" Caption="Warehousetype" Width="0" />
                            <dxe:ListBoxColumn FieldName="IsComponent" Caption="IsComponent" Width="0" />
                            <dxe:ListBoxColumn FieldName="ComponentProduct" Caption="ComponentProduct" Width="0" />
                        </Columns>
                        <ClientSideEvents ValueChanged="function(s, e) {GetProductDetails()}" KeyDown="ProductlookUpKeyDown" />
                    </dxe:ASPxComboBox>
    ...
    <div>
        <dxe:ASPxPopupControl ID="popup_Tagging" runat="server" ClientInstanceName="cpopup_Tagging"
            ...
            <ContentCollection>
                <dxe:PopupControlContentControl runat="server">
                    <dxe:ASPxCallbackPanel runat="server" ID="ComponentQuotationPanel" ClientInstanceName="cQuotationComponentPanel" OnCallback="ComponentQuotation_Callback">
                        <PanelCollection>
                            <dxe:PanelContent runat="server">
                                <dxe:ASPxGridLookup ID="lookupOrder" SelectionMode="Multiple" runat="server" ClientInstanceName="clookupOrder"
                                    KeyFieldName="PurchaseOrder_Id" Width="100%" TextFormatString="{0}" AutoGenerateColumns="False" MultiTextSeparator=", "
                                    OnDataBinding="lookupOrder_DataBinding">
                                    <Columns>
    ...
    <asp:SqlDataSource ID="VendorDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:crmConnectionString %>" />
