    ...
    public partial class PurchaseOrder : System.Web.UI.Page
    {
        ...
        public bool BindSessionByDatatable(DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string SalePrice, Discount, Amount, TaxAmount, TotalAmount, Order_Num1, ProductName, IsComponent, IsLinkedProduct;
                ...
                purChaseDT.Rows.Add(Convert.ToString(i + 1), Convert.ToString(i + 1), Convert.ToString(dt.Rows[i]["QuoteDetails_ProductId"]), Convert.ToString(dt.Rows[i]["QuoteDetails_ProductDescription"]),
                    Convert.ToString(dt.Rows[i]["QuoteDetails_Quantity"]), Convert.ToString(dt.Rows[i]["UOM_ShortName"]), "", Convert.ToString(dt.Rows[i]["QuoteDetails_StockQty"]), Convert.ToString(dt.Rows[i]["UOM_ShortName"]),
                               SalePrice, Discount, Amount, TaxAmount, TotalAmount, "U", Convert.ToInt64(dt.Rows[i]["Indent_No"]), Order_Num1, ProductName, IsComponent, IsLinkedProduct);
            }
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
                    ...
                    if (ProductDetails != "" && ProductDetails != "0")
                    {
                        ...
                        if (Isexists == false)
                        {
                            PurchaseOrderdt.Rows.Add(SrlNo, OrderDetails_Id, ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM,
                                PurchasePrice, Discount, Amount, TaxAmount, TotalAmount, "U", Indent_Id, ProductName, IsComponentProduct, IsLinkedProduct);
                        }
                        if (IsComponentProduct == "Y")
                        {
                            DataTable ComponentTable = objPurchaseOrderBL.GetComponantProduct(ProductID);
            ...
            if (IsDeleteFrom != "D" && IsDeleteFrom != "C")
            {
               ...
               .Where(gr => gr.Count() > 1)
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "UdfMandetory" || validate == "transporteMandatory" || validate == "TCMandatory" || validate == "nullQuantity" || validate == "duplicateProduct")
                {
                ...
                else
                {
                    if (IsPOExist(IndentRequisitionNo))
                    {
                        ...
                        //TaxDetaildt = gstTaxDetails.SetTaxTableDataWithProductSerialForPurchaseRoundOff(ref tempQuotation, "SrlNo", "ProductID", "Amount", "TaxAmount", "TotalAmount", TaxDetaildt, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, TaxType, Convert.ToString(lookup_Customer.Value));
                        ...
                        tempWarehousedt, TaxDetaildt, tempTaxDetailsdt, tempBillAddress, approveStatus, strCreditDays, strPoDate) == false)
                        {
        ...
        string approveStatus, string strCreditDays, string strPoDate)
        {
            try
            {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    if (!string.IsNullOrEmpty(IdKey))
                    {
                        if (IdKey != "ADD")
                        {
                            dt_QuotationDetails = objPurchaseOrderBL.GetIndentDetailsFromPO(QuoComponent, IdKey, "");
                        }
                        else
                        {
