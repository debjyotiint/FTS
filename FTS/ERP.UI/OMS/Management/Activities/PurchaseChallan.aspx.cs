    ...
    public partial class PurchaseChallan : ERP.OMS.ViewState_class.VSPage//ERP.OMS.ViewState_class.VSPage//System.Web.UI.Page//PersistViewStateToFileSystem
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                DataTable DT = objEngine.GetDataTable("Config_SystemSettings", " Variable_Value ", " Variable_Name='BulkGRN' AND IsActive=1");
                if (DT != null && DT.Rows.Count > 0)
                {
                    string IsMandatory = Convert.ToString(DT.Rows[0]["Variable_Value"]).Trim();
                    if (IsMandatory == "Yes")
                    {
                        btnUploadSerial.Visible = true;
                    }
                    else
                    {
                        btnUploadSerial.Visible = false;
                    }
                }
                else
                {
                ...
                else
                {
                   ...
                   Session["PCTaxDetails"] = GetTaxDataWithGST(GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd")));
                    ...
                    {
                        IsExistsDocumentInERPDocApproveStatus(key);
                    }
        ...
        public void FillGrid()
        {
            ...
            if (PurchaseOrderEditdt != null && PurchaseOrderEditdt.Rows.Count > 0)
            {
                ...
                string VendorName = Convert.ToString(PurchaseOrderEditdt.Rows[0]["VendorName"]);//5
                    ...
                    else
                    {
                        ...
                        if (!IsPcTransactionExist(PurchaseOrder_Id))
                        {
                            ...
                            if (IsPOExist(IndentRequisitionNo))
                            {
                                ...
                                TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialForPurchaseRoundOff(ref tempQuotation, "SrlNo", "ProductID", "Amount", "TaxAmount", "TotalAmount", TaxDetailTable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, TaxType, Convert.ToString(hdnCustomerId.Value));
                                            ...
                                            ref int strIsComplete, ref int strChallanID)
        {
            try
            {
                ...
                // Added by Sam on 01112017 to distinguish between vendor and customer Start
                cmd.Parameters.AddWithValue("@Entitytype", "DV");
                // Added by Sam on 01112017 to distinguish between vendor and customer End
        ...
        protected void GrdWarehouse_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "SaveSerialData")
            {
                ...
                DataTable dt_SerialData = new DataTable();
                if( Session["GRN_Serial"] !=null)
                {
                    dt_SerialData = (DataTable)Session["GRN_Serial"];
                }
                else
                {
                    dt_SerialData.Columns.Add("Serial", typeof(string));
                }
                ...
                if (Type == "WS")
                {
                    ...
                    {
                        foreach (DataRow Serialrow in dt_SerialData.Rows)
                        {
                            string SerialNo = Convert.ToString(Serialrow[0]);
                            ...
                            if (SerialNoCount_Local.Length == 0)
                            {
                            ...
                            else
                            {
                                if(duplicateSerial=="")
                                {
                                    duplicateSerial = SerialNo;
                                }
                                else
                                {
                                    duplicateSerial = duplicateSerial + ", " + SerialNo;
                                }

                                GrdWarehouse.JSProperties["cpduplicateMsg"] = "_duplicateSerial";
                                GrdWarehouse.JSProperties["cpduplicateSerial"] = duplicateSerial;
                            }
                        }
                    }
                }
                else if (Type == "S")
                {
                    ...
                    if (editWarehouseID == "0")
                    {
                        foreach (DataRow Serialrow in dt_SerialData.Rows)
                        {
                            string SerialNo = Serialrow["Serial"].ToString();
...
//            comboBox.DataBind();
//        }
//        protected void SetVendorDDbyValue(string vendorId)
//        {
//            VendorDataSource.SelectCommand = @" Select cnt_internalid,IsNull(cnt_shortName,'NA') ShortName,IsNull(cnt_firstName,'')+IsNull(cnt_middleName,'')+IsNull(cnt_lastName,'')  Name 
//                                                From tbl_master_contact Where (cnt_internalid = @ID) ";
//            VendorDataSource.SelectParameters.Clear();
//            VendorDataSource.SelectParameters.Add("ID", TypeCode.String, vendorId);
