...
{
    public partial class management_Activities_futuresale : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SalesDetailsGrid.SettingsCookies.CookiesID = "BreeezeErpGridCookiesSalesFutureDetailsGrid";
                this.Page.ClientScript.RegisterStartupScript(GetType(), "heightL", "<script>addCookiesKeyOnStorage('BreeezeErpGridCookiesSalesFutureDetailsGrid');</script>");
                ...
                {
                    if (Session["Fromdate_S"] != null && Session["ToDate_S"] != null)
                    {

                        ASPxFromDate.Text = Convert.ToDateTime(Session["Fromdate_S"]).ToString("dd-MM-yyyy");
                        ASPxToDate.Text = Convert.ToDateTime(Session["ToDate_S"]).ToString("dd-MM-yyyy");
                        dtFrom = Convert.ToDateTime(ASPxFromDate.Date);
                        dtTo = Convert.ToDateTime(ASPxToDate.Date);
                    }
        ...
        protected void SalesDetailsGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            dtFrom = Convert.ToDateTime(ASPxFromDate.Date);
            dtTo = Convert.ToDateTime(ASPxToDate.Date);
            Session["Fromdate_S"] = dtFrom;
            Session["ToDate_S"] = dtTo;
        ...
        protected void btMyPendingTask_Click(object sender, EventArgs e)
        {
            if (Session["cntId"] != null)
            {
                ...
                Response.Redirect("../Master/PendingTaskReport.aspx?Salsmanid=" + salsmanId + "&returnId=3");
            }
            else
            {
        ...
        protected void btMyPendingActivity_Click(object sender, EventArgs e)
        {
            if (Session["cntId"] != null)
            {
                ...
                Response.Redirect("TodayTask.aspx?Id=3&frmdate=" + frmdate + "&todate=" + todate);
            }
            else
            {
        ...
        void BindGrid(DateTime FromDate, DateTime ToDate)
        {
            ...
            SalesDetailsGridDataSource.SelectParameters.Add("ActivityTypeId", "3");
        ...
        protected void SalesDetailsGrid_HtmlRowCreated(object sender, ASPxGridViewTableRowEventArgs e)
        {
            ...
            if (Session["export"] == null)
            {
                ...
                {
                    hpnPh.Visible = true;
                    hpnPh.NavigateUrl = "../CRMPhoneCallWithFrame.aspx?TransSale=" + salesid + "&Assigned=" + AssignedById + "&type=" + type + "&Cid=" + CusId + "&Pid=3";
                }
                else { hpnPh.Visible = false; }
                if (containsSalesVisit)
                {
                    hpnSv.NavigateUrl = "CRMSalesVisitWithIFrame.aspx?TransSale=" + salesid + "&Assigned=" + AssignedById + "&type=" + type + "&Cid=" + CusId + "&Pid=3";
                    hpnSv.Visible = true;
                }
                ...
                if (containsSms)
                {
                    hpnOtheractvSms.NavigateUrl = "CRMOtherActivities.aspx?TransSale=" + salesid + "&TypId=3" + "&Assigned=" + AssignedById + "&type=" + type + "&Cid=" + CusId + "&Pid=3";
                    hpnOtheractvSms.Visible = true;
                }
                else { hpnOtheractvSms.Visible = false; }
                if (containsEmail)
                {
                    hpnOtheractvEmail.NavigateUrl = "CRMOtherActivities.aspx?TransSale=" + salesid + "&TypId=2" + "&Assigned=" + AssignedById + "&type=" + type + "&Cid=" + CusId + "&Pid=3";
                    hpnOtheractvEmail.Visible = true;
                }
                else { hpnOtheractvEmail.Visible = false; }
                if (containsMeeting)
                {
                    hpnOtheractvMeet.NavigateUrl = "CRMOtherActivities.aspx?TransSale=" + salesid + "&TypId=5" + "&Assigned=" + AssignedById + "&type=" + type + "&Cid=" + CusId + "&Pid=3";
        ...
        public void bindSalesActivityDetailsexport(int Filter)
        {
            ...
            exporter.FileName = "SalesActivity_FutureDetails";

            exporter.PageHeader.Left = "Sales Activity Future Details";
        ...
        protected void btMyactivities_Click(object sender, EventArgs e)
        {
            ...
            {
                int salsmanId = Convert.ToInt32(Session["cntId"]);
                Response.Redirect("../Master/DailySalesReport.aspx?Salsmanid=" + salsmanId + "&returnId=3");
            }
            else
            {
