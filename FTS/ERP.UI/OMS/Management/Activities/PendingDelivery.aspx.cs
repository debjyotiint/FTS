...
{
    public partial class PendingDelivery : ERP.OMS.ViewState_class.VSPage//System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/PendingDelivery.aspx");
            if (!IsPostBack)
            {
        ...
        protected void GrdOrder_DataBinding(object sender, EventArgs e)
        {
            ...
            if (HttpContext.Current.Session["LastCompany"] != null)
            {
                ...
                if (HttpContext.Current.Session["userbranchHierarchy"] != null)
                {
                    string userbranch = Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]);
                    //dtdata = objCRMSalesOrderDtlBL.GetSalesInvoiceOnPendingDeliveryList(userbranch, lastCompany);
                    dtdata = objCRMSalesOrderDtlBL.GetSalesInvoiceOnUnDeliveryListByDateFiltering(userbranch, lastCompany, BranchID, FromDate, ToDate);
                }
            }
        ...
        public void bindexport(int Filter)
        {
            ...
            string filename = "Undelivered_List";
            exporter.FileName = filename;
            exporter.FileName = "Undelivered_List";

            exporter.PageHeader.Left = "Undelivered Challan List";
            ...
            else if (WhichCall == "SaveTransporter")//Newly Added for saving Transporter
            {
                if (!string.IsNullOrEmpty(WhichType))
                {
                    if (!string.IsNullOrEmpty(hfControlData.Value))
                    {
                        ...
                        objCommonBL.InsertDeliveryReceiptTransporterControlDetails(Convert.ToInt32(WhichType), "SC", hfControlData.Value, Convert.ToString(HttpContext.Current.Session["userid"]));
