...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script>
        ...
        function OnMoreInfoClick(keyValue) {
            var URL = "PurchaseOrder.aspx?key=" + keyValue + "&req=V&status=PO";
            ...
            if (ExpectedDeliveryDate.format('yyyy-MM-dd') != requisitionDate.format('yyyy-MM-dd'))
                if (ExpectedDeliveryDate < requisitionDate) {
                    jAlert('Expected Delivery date must be same or later to Requisition Date. Cannot Proceed.', 'Alert', function () {
                        ...
                        //InsgridBatch.GetEditor('ExpectedDeliveryDate').focus();
        ...
        function CustomButtonClick(s, e) {
            ...
            if (e.buttonID == 'CustomBtnDelete') {
                ...
                //VisibleIndexE = e.visibleIndex;
                //s.GetRowValues(e.visibleIndex, 'ValueDate;TransactionDate;MaxLockDate;IBRef', OnGetRowValuesOnDelete);
            }
        ...
        function SaveExitButtonClick() {
            ...
            for (var i = 0; i <= InsgridBatch.GetVisibleRowsOnPage() ; i++) {
                ...
                //if (document.getElementById('hdn_Mode').value == 'Entry') {
            ...
            if (InsgridBatch.GetVisibleRowsOnPage() > 0) {
                ...
                //}
                //else {
                //    jAlert('You must enter atleast one record to proceed.');
                //}
                if (IsType == "Y") {
                    // InsgridBatch.batchEditApi.EndEdit();
                    //InsgridBatch.UpdateEdit();
        ...
        function AutoCalValue(s, e) {
            ...
            // var ExchangeRate = ctxtRate.GetValue();
            //if (Currency == basedCurrency[0]) {
            //    InsgridBatch.GetEditor('gvColValue').SetValue(Quantity * Rate);
            //}
            //else {
            //    InsgridBatch.GetEditor('gvColValue').SetValue(Quantity * Rate * ExchangeRate);
            //}
...
$(document).ready(function () {
    ...
    //$('#txtRate_I').blur(function () {
    //    if (InsgridBatch.GetVisibleRowsOnPage() == 1) {
    //        InsgridBatch.batchEditApi.StartEdit(-1, 2);
    //    }
    //})
    //if ($("#txtRate_I").prop('disabled') == false) {

    //    $('#CmbCurrency_I').blur(function () {
    //        if (InsgridBatch.GetVisibleRowsOnPage() == 1) {
    //            InsgridBatch.batchEditApi.StartEdit(-1, 2);
    //        }
    //    })
    ...
    <script>
       ...
       <%--     $('#<%= lblStkUOMTo.ClientID %>').text(strUOM);--%>
        ...
        function ProductSelected(s, e) {
            ...
            <%--if (strProductID != "" && strProductID != "0") {
                $.ajax({
                    ...
                    success: function (msg) {
                       ...
                       // console.log('global Row Index on ajax', globalRowIndex);
                        return;
                    }
                });
            } --%>
            ...
            cacpAvailableStock.PerformCallback(strProductID);
            InsgridBatch.batchEditApi.StartEdit(globalRowIndex, 5);
            // console.log('global Row Index', globalRowIndex);
            // return;
        }
    </script>
    ...
    <div class="form_main">
        ...
        <div id="DivEntry" style="display: none">
            <div style="background: #f5f4f3; padding: 8px 0; margin-bottom: 15px; border-radius: 4px; border: 1px solid #ccc;" class="clearfix">
                ...
                <div class="col-md-3">
                    <label>Indent/Requisition No.<span style="color: red;">*</span></label>
                    <div>
                    ...
                    <Columns>
                        ...
                        <dxe:GridViewDataTextColumn VisibleIndex="4" Caption="Quantity" FieldName="gvColQuantity" Width="110" HeaderStyle-HorizontalAlign="Right">
                            <PropertiesTextEdit FocusedStyle-HorizontalAlign="Right" Style-HorizontalAlign="Right">
                                ...
                                <%--  <ClientSideEvents LostFocus="AutoCalValue"  GotFocus="ProductsGotFocus" />--%>
        ...
        <div id="DivEdit">
            ...
            <%-- SettingsCookies-Enabled="true" SettingsCookies-StorePaging="true"
                ...
                <Columns>
                    ...
                    <dxe:GridViewDataTextColumn VisibleIndex="1" Caption="Indent/Requisition No." FieldName="Indent_RequisitionNumber">
                        <CellStyle Wrap="False" CssClass="gridcellleft"></CellStyle>
                    </dxe:GridViewDataTextColumn>
                    <dxe:GridViewDataTextColumn VisibleIndex="2" Caption="Date" FieldName="Indent_RequisitionDate" Width="120">
                         <%--<PropertiesTextEdit DisplayFormatString="dd-MM-yyyy"
                            DisplayFormatInEditMode="True"></PropertiesTextEdit>--%>
                        <CellStyle Wrap="False" CssClass="gridcellleft"></CellStyle>
                    </dxe:GridViewDataTextColumn>
                    <dxe:GridViewDataTextColumn VisibleIndex="3" Caption="From Branch" FieldName="Indent_branch">
                        <CellStyle Wrap="False" CssClass="gridcellleft"></CellStyle>
                    </dxe:GridViewDataTextColumn>
