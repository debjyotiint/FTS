...
{
    public partial class PurchaseChallanList_OD : ERP.OMS.ViewState_class.VSPage
    {
        ...
        public DataTable GetPurchaseChallanListGridData(string Branch, string company, string finyear, string BranchID, DateTime FromDate, DateTime ToDate)
        {

            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_Purchasechallan_Details");
            proc.AddVarcharPara("@Action", 500, "StockTransfer");
        ...
        protected void Grid_PurchaseChallan_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Command == "Delete")
            {
                ...
                {
                    deletecnt = DeletePurchaseChallan(POID, Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]));
            ...
            else if (Command == "FilterGridByDate")
            {
                ...
                dtdata = GetPurchaseChallanListGridData(userbranch, lastCompany, finyear, BranchID, FromDate, ToDate);
        ...
        public int DeletePurchaseChallan(string pcid, string comapanyid, string finyear)
        {
            ...
            if (!string.IsNullOrEmpty(pcid) && !string.IsNullOrEmpty(comapanyid) && !string.IsNullOrEmpty(finyear))
            {
                DataTable dt = new DataTable();
                ProcedureExecute proc = new ProcedureExecute("prc_Purchasechallan_Details");
                proc.AddVarcharPara("@Action", 100, "DeleteStockTransferAll");
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_PurchaseChallanLists
                            ...
                            && branchidlist.Contains(Convert.ToInt32(d.BranchID)) && Convert.ToString(d.Doc_Type) == "SC"
