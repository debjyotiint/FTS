...
namespace ERP.OMS.Management.Activities
{
    public partial class PurchaseInvoiceForCustomer : VSPage //: System.Web.UI.Page
    {
        ...
        public void getVendorDetail()
        {
            ...
            lookup_Customer.DataSource = (DataTable)Session["VendorList_PB"];
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/PurchaseInvoiceListForCustomer.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                //{
                //    IsUdfpresent.Value = Convert.ToString(dst.Tables[3].Rows[0]["Variable_Value"]);
                //}



                //DataTable dtbtn = objPurchaseInvoice.PurchaseOrdTagInPurchaseInvoice("PurchaseOrdTagInPurchaseInvoice");
                ...
                //}
                //DataTable dtbranch = objPurchaseInvoice.BranchEditableInPurchaseInvoice("PB_Branch_Selection");
                //if (dtbranch.Rows.Count > 0)
                //{
                //    string settingpbbranch = Convert.ToString(dtbranch.Rows[0]["Variable_Value"]);
                //    if (settingpbbranch.ToUpper().Trim() == "NO")
                //    {
                //        ddl_Branch.Enabled = false; 
                //    } 
                ...
                if (Request.QueryString["key"] == "ADD")
                {
                    ...
                    if (Session["SaveModePB"] != null)  // it has been removed from coding side of Quotation list 
                    {
                        ...
                        //}
                        if (Session["CLschemavaluePB"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            ddl_numberingScheme.SelectedValue = Convert.ToString(Session["CLschemavaluePB"]); // it has been removed from coding side of Quotation list 
                            string[] selectedbranch = new string[] { };
                            selectedbranch = Convert.ToString(Session["CLschemavaluePB"]).Split('~');
                ...
                else
                {
                        ...
                        inventorytype, productcharges, partyInvNo, partyInvDate, ReverseTaxtable, strenteredBranchId) == true)
                    {
                        ...
                        if (ActionType == "Add") // session has been removed from quotation list page working good
                        {
                            ...
                            Session["CLschemavaluePB"] = schemavalue;        // session has been removed from quotation list page working good
