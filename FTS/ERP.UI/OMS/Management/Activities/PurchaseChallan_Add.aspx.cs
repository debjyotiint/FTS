    ...
    public partial class PurchaseChallan_Add : ERP.OMS.ViewState_class.VSPage//System.Web.UI.Page
    {
            ...
            if (Session["userid"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        DataTable Calculate_dt = ds_GRNDetails.Tables[4];
        ...
        public void Fill_GRNDetails(DataTable dt)
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //string PurchaseOrder_SalesmanId = Convert.ToString(dt.Rows[0]["PurchaseChallan_SalesmanId"]);
                //string PurchaseOrder_IndentDate = Convert.ToString(dt.Rows[0]["PurchaseChallan_PurchaseOrderDate"]);
                ...
                SetVendorDDbyValue(Customer_Id);
                ...
                VendorComboBox.ClientEnabled = false;
                ...
                lookupOrder.GridView.Selection.CancelSelection();
                lookupOrder.DataSource = IndentTable;
                lookupOrder.DataBind();


                string[] eachQuo = Convert.ToString(dt.Rows[0]["PurchaseChallan_PurchaseOrderId"]).Split(',');
                if (eachQuo.Length > 1)//More tha one quotation
                {
                    ...
                    foreach (string val in eachQuo)
                    {
                        ...
                        {
                            lookupOrder.GridView.Selection.SelectRowByKey(Convert.ToInt32(val));
                        }
                    }
                }
                else if (eachQuo.Length == 1)//Single Quotation
                {
                    foreach (string val in eachQuo)
                    {
                        if (!string.IsNullOrEmpty(val))
                        {
                            ...
                            int VisibleIndex = lookupOrder.GridView.FindVisibleIndexByKeyValue(val);
                            lookupOrder.GridView.FocusedRowIndex = VisibleIndex;
                        }
                    }
                }
            }
        ...
        #region Predictive Vendor

        protected void VendorComboBox_ItemsRequestedByFilterCondition(object source, ListEditItemsRequestedByFilterConditionEventArgs e)
        {
            if (Convert.ToString(e.Filter).Trim() != "")
            {
                ...
                VendorDataSource.SelectCommand = @" Select cnt_internalid,ShortName,Name From
                                                    (Select *,row_number() Over(Order By [Name] Asc) as [rn] From
                                                    (Select cnt_internalid,IsNull(cnt_shortName,'NA') ShortName,IsNull(cnt_firstName,'')+IsNull(cnt_middleName,'')+IsNull(cnt_lastName,'')  Name 
                                                    From tbl_master_contact
                                                    Where cnt_contactType ='DV' AND cnt_internalId in (Select Ven_InternalId from tbl_master_VendorBranch_map Where branch_id in(@branchID,'0'))) as tbl
                                                    Where ([ShortName] + ' ' + [Name]) LIKE @filter) as st where st.[rn] between @startIndex and @endIndex";
                ...
                VendorDataSource.SelectParameters.Add("branchID", TypeCode.String, strBranch);
                comboBox.DataSource = VendorDataSource;
                comboBox.DataBind();
            }
        }
        protected void VendorComboBox_ItemRequestedByValue(object source, ListEditItemRequestedByValueEventArgs e)
        {
            long value = 0;
            if (e.Value == null || !Int64.TryParse(e.Value.ToString(), out value))
                return;
            ASPxComboBox comboBox = (ASPxComboBox)source;
            VendorDataSource.SelectCommand = @" Select cnt_internalid,IsNull(cnt_shortName,'NA') ShortName,IsNull(cnt_firstName,'')+IsNull(cnt_middleName,'')+IsNull(cnt_lastName,'') Name 
                                                From tbl_master_contact Where (cnt_internalid = @ID) ";
        ...
        protected void SetVendorDDbyValue(string customerId)
        {
            VendorDataSource.SelectCommand = @" Select cnt_internalid,IsNull(cnt_shortName,'NA') ShortName,IsNull(cnt_firstName,'')+IsNull(cnt_middleName,'')+IsNull(cnt_lastName,'')  Name 
                                                From tbl_master_contact Where (cnt_internalid = @ID) ";
            VendorDataSource.SelectParameters.Clear();
            VendorDataSource.SelectParameters.Add("ID", TypeCode.String, customerId);

            VendorComboBox.DataSource = VendorDataSource;
            VendorComboBox.DataBind();
            VendorComboBox.Value = customerId;
        }
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            foreach (var args in e.InsertValues)
            {
                ...
                if (Product_ID != "" && Product_ID != "0")
                {
                    ...
                    {
                        DataTable ComponentTable = objSalesInvoiceBL.GetLinkedProductList("ComponentProduct", ProductID);
                        foreach (DataRow Comp_dr in ComponentTable.Rows)
                        {
                            ...
                            Transactiondt.Rows.Add(Convert.ToString(Convert.ToInt32(SrlNo) + 1), Convert.ToString(Convert.ToInt32(SrlNo) + 1), DocNumber, DocID,
            ...
            if (IsDeleteFrom != "D" && IsDeleteFrom != "C")
            {
                ...
                string strVendor = Convert.ToString(VendorComboBox.Value);
                ...
                List<object> QuoList = lookupOrder.GridView.GetSelectedFieldValues("PurchaseOrder_Id");
               ...
               // #region Check Mandatory - Duplicate Serial
               ...
               // DataTable duplicateSerial = GetDuplicateDetails(PurchaseOrder_Id, Transactiondt, tempWarehousedt, "Final");
               ...
               // #endregion
                ...
                else
                {
                    if (!IsPcTransactionExist(PurchaseOrder_Id))
                    {
                        ...
                        TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialForPurchaseRoundOff(ref Transactiondt, "SrlNo", "ProductID", "TotalAmount", "TaxAmount", "NetAmount", TaxDetailTable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, TaxType, Convert.ToString(VendorComboBox.Value));
                        ...
                        AddModifyPurchaseOrder(ActionType,PurchaseOrder_Id,strIsInventory,strSchemeType,strPurchaseNumber,strPurchaseDate,strBranch,strVendor,strContactName,Reference,
                                                ...
                                                Transactiondt, tempWarehousedt, TaxDetailTable, tempTaxDetailsdt, tempBillAddress, ref strIsComplete, ref strChallanID, ref strChallanNumber);
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    DataTable dt_QuotationDetails = objSalesInvoiceBL.GetSelectedComponentProductList("GetSeletedPurchaseOrderProducts", QuoComponent, strInvoiceID);
        ...
        protected void cgridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (e.Parameters.Split('~')[0] != "SaveGST")
            {
                ...
                GetVendorGstin.AddVarcharPara("@Action", 500, "GetVendorGSTINByBranch");
                GetVendorGstin.AddVarcharPara("@branchId", 10, Convert.ToString(ddl_Branch.SelectedValue));
                GetVendorGstin.AddVarcharPara("@entityId", 10, Convert.ToString(VendorComboBox.Value));
                DataTable VendorGstin = GetVendorGstin.GetTable();
        ...
        protected void lookupOrder_DataBinding(object sender, EventArgs e)
        {
            if (Session["PORequiData"] != null)
            {
                lookupOrder.DataSource = (DataTable)Session["PORequiData"];
            }
        }
        protected void ComponentQuotation_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (e.Parameter.Split('~')[0] == "BindComponentGrid")
            {
                ...
                {
                    lookupOrder.GridView.Selection.UnselectAll();
                }
                ...
                lookupOrder.GridView.Selection.CancelSelection();
                lookupOrder.DataSource = ComponentTable;
                lookupOrder.DataBind();
