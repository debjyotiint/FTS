...
{
    public partial class PurchaseInvoiceListForCustomer : ERP.OMS.ViewState_class.VSPage
    {
        ...
        public void PopulateGridByFilter(string  fromdate, string toDate, string branch)
        {
            ...
            //dtdata = objPurchaseInvoice.GetInvoiceListGridDataByDate(Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]), Convert.ToString(HttpContext.Current.Session["LastCompany"]), fromdate, toDate, branch,"CL");
            ////if (dtdata != null)
            ////{
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_PBLists
                            ...
                            && branchidlist.Contains(Convert.ToInt32(d.branchid)) && d.invoicefor=="CL"
        ...
        {
            //DataTable dtdata = new DataTable();
            //dtdata = objPurchaseInvoice.GetPurchaseInvoiceListGridData(userbranch, lastCompany, lastfinyear,"CL");
