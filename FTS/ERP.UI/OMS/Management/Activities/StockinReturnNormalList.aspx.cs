...
{
    public partial class StockinReturnNormalList : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/StockinReturnNormalList.aspx");

            if (!IsPostBack)
            {
                //Session.Remove("StockInReturnListingDetails");
                if (Session["LastCompany"] != null && Session["LastCompany"] != null)
                {
                    ...
                    //  GetSalesReturnListGridData(userbranch, lastCompany, FinyearStartDate, FinYearEndDate, ubranch);

                    GrdSalesReturn.SettingsCookies.CookiesID = "BreeezeErpGridCookiesSalesStockinReturnNormalListDetailsGrid";
                    this.Page.ClientScript.RegisterStartupScript(GetType(), "heightL", "<script>addCookiesKeyOnStorage('BreeezeErpGridCookiesSalesStockinReturnNormalListDetailsGrid');</script>");
        ...
        public void bindexport(int Filter)
        {
            ...
            GrdSalesReturn.Columns[18].Visible = false;
            string filename = "Receive Against Normal Return";
            exporter.FileName = filename;
            //  exporter.FileName = "GrdSalesStockinReturnNormal";
            exporter.Landscape = true;
            exporter.PaperKind = System.Drawing.Printing.PaperKind.A4;
            exporter.MaxColumnWidth = 200;
            exporter.PageHeader.Left = "Receive Against Normal Return";
        ...
        protected void GrdSalesReturn_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Command == "Delete")
            {
                ...
                deletecnt = objSalesReturnBL.DeleteSalesReturn(SalesReturnID, Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), "SISRN", Convert.ToString(Session["userbranchID"]));
                ...
                dtdata = objSalesReturnBL.GetSalesReturnStockInNormalListGridData(Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]), Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), FromDate, ToDate, ubranch, BranchID);

                if (dtdata != null && dtdata.Rows.Count > 0)
                {
                    //  Session["StockInReturnListingDetails"] = dtdata;
            ...
            else if (Command == "FilterGridByDate")
            {
                ...
                dtdata = objSalesReturnBL.GetSalesReturnStockInNormalListGridData(userbranch, lastCompany, finyear, FromDate, ToDate, ubranch, BranchID);
                //  Session["StockInReturnListingDetails"] = dtdata;
                if (dtdata != null && dtdata.Rows.Count > 0)
                {
        ...
        public void GetSalesReturnListGridData(string userbranch, string lastCompany, string FinyearStartDate, string FinYearEndDate, string uBranch)
        {
            //DataTable dtdata = new DataTable();

            //string finyear = Convert.ToString(Session["LastFinYear"]);
            //dtdata = objSalesReturnBL.GetSalesReturnStockInNormalListGridData(userbranch, lastCompany, FinyearStartDate, FinYearEndDate, uBranch, finyear);
            //if (dtdata != null && dtdata.Rows.Count > 0)
            //{
            //    GrdSalesReturn.DataSource = dtdata;
            //    GrdSalesReturn.DataBind();
        ...
        //  DataTable dsdata = (DataTable)Session["StockInReturnListingDetails"];
        ...
        protected void EntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_StockInReturnNormalLists
