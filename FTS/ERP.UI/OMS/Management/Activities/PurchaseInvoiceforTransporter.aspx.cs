...
{
    public partial class PurchaseInvoiceforTransporter : VSPage //: System.Web.UI.Page
    {
        ...
        public void getVendorDetail()
        {
            ...
            if (Session["TransporterList_PB"] == null)
            {
                Session["TransporterList_PB"] = dt;
            }

            lookup_Customer.DataSource = (DataTable)Session["TransporterList_PB"];
        ...
        {
            string branch = e.Parameter.Split('~')[0];
        ...
        //       Session["TransporterList_PB"] = null;  in Page Load
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
               dt_PLQuote.ClientEnabled = false;
                ...
                Session["PurchaseOrderFinalTaxRecord"] = null;
                ...
                Session["PBwarehousedetailstempNew"] = null;
                Session["WbranchInEdit"] = null;
                Session["PBNonInvProChg"] = null;
                ViewState["EnteredBranchID"] = null;
                Session["EnteredBranchID"] = null;
                Session["ProductDetailsList_PB"] = null;
                Session["TransporterList_PB"] = null;
                ...
                if (Request.QueryString["key"] == "ADD")
                {
                    ...
                    if (Session["TRSaveModePB"] != null)  // it has been removed from coding side of Quotation list 
                    {
                        ...
                        //}
                        if (Session["TRschemavaluePB"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            ddl_numberingScheme.SelectedValue = Convert.ToString(Session["TRschemavaluePB"]); // it has been removed from coding side of Quotation list 
                            string[] selectedbranch = new string[] { };
                            selectedbranch = Convert.ToString(Session["TRschemavaluePB"]).Split('~');
                            ddl_Branch.SelectedValue = selectedbranch[1];

                        }
                        

                        if (Convert.ToString(Session["TRSaveModePB"]) == "A")
                        {
                            dt_PLQuote.Focus();
                            txtVoucherNo.Enabled = false;
                            txtVoucherNo.Text = "Auto";
                        }
                        else if (Convert.ToString(Session["TRSaveModePB"]) == "M")
                        {
                ...
                else
                {
                    ...
                    lblHeading.Text = "Modify Purchase Invoice for Transporter";
        ...
        public bool CheckPartyNo(string vendorid, string partyno)
        {
            ...
            if (vendorid != "" && Convert.ToString(partyno).Trim() != "")
            {
                ...
                status = objMShortNameCheckingBL.CheckUniquePartyNo(Convert.ToString(vendorid).Trim(), partyno, "PurchaseInvoice_CheckPartyNo", mode, pbid);
                if (status)
                {
                    cnt = 1;
                }
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (IsDeleteFrom != "D" && IsCompFrom != "C")
            {
                ...
                if (validate == "AddressProblem" || validate == "outrange" || validate == "nullQuantity" || validate == "duplicateProduct" || validate == "duplicate" || validate == "transporteMandatory" || validate == "TCMandatory")
                {
                ...
                else
                {
                    ...
                    if (Convert.ToString(ddl_AmountAre.Value).Trim() != "4")
                    {
                        if (!chk_reversemechenism.Checked)
                        {
                            ...
                            TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialForPurchaseRoundOff(ref tempPurchaseInvoice, "SrlNo", "ProductID", "Amount", "TaxAmount", "TotalAmount", TaxDetailTable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strenteredBranchId, ShippingState, incluexclutype, Convert.ToString(lookup_Customer.Value));
                            //TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialForPurchaseRoundOff(ref tempPurchaseInvoice, "SrlNo", "ProductID", "Amount", "TaxAmount", TaxDetailTable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, incluexclutype, Convert.ToString(lookup_Customer.Value));
                        }
                        else
                        {
                            string RShippingState = BillingShippingControl.GetShippingStateCode();
                            ReverseTaxtable = gstTaxDetails.GetReverseTaxTable(tempPurchaseInvoice, "SrlNo", "ProductID", "Amount", "TaxAmount", ReverseTaxtable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strenteredBranchId, RShippingState, incluexclutype, Convert.ToString(lookup_Customer.Value));
                            //ReverseTaxtable = gstTaxDetails.GetReverseTaxTable(tempPurchaseInvoice, "SrlNo", "ProductID", "Amount", "TaxAmount", ReverseTaxtable, "P", dt_PLQuote.Date.ToString("yyyy-MM-dd"), strBranch, RShippingState, incluexclutype, Convert.ToString(lookup_Customer.Value));
                        }
                    }
                    ...
                    if (ModifyQuatation(MainPurchaseInvoiceID, strSchemeType, UniquePurchaseInvoice, strQuoteDate, IndentRequisitionDate,
                        ...
                        inventorytype, productcharges, partyInvNo, partyInvDate, ReverseTaxtable, strenteredBranchId) == true)
                    {
                        ...
                        if (ActionType == "Add") // session has been removed from quotation list page working good
                        {
                            ...
                            Session["TRschemavaluePB"] = schemavalue;        // session has been removed from quotation list page working good
                            ...
                            if (schematype == "Auto")
                            {
                                Session["TRSaveModePB"] = "A";
                            }
                            else
                            {
                                Session["TRSaveModePB"] = "M";
                            }
                        }
        ...
        public bool ModifyQuatation(string PurchaseInvoiceID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strQuoteExpiry, string strCustomer, string strContactName,
            ...
            string partyInvNo, string partyInvDate, DataTable ReverseTaxtable, string strenteredBranchId)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@InvoiceFor", "TR");
        ...
        public void SetPBDetails() // To Get Main Header Section Detail in Edit Mode
        {
            ...
            if (PBEditdt != null && PBEditdt.Rows.Count > 0)
            {
               ...
               //if (Session["TransporterList_PB"] == null)
               //{
                   Session["TransporterList_PB"] = vendordt;
               //}
        ...
        protected void acbpCrpUdf_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            //string vendorid = Convert.ToString(lookup_Customer.GridView.GetSelectedFieldValues("cnt_internalid"));
        ...
        //    {
        //        DataTable TaxRecord = (DataTable)Session["PBNonInvProChg"];
        //        int slNo = Convert.ToInt32(HdSerialNo.Value);
        //        //For GST/CST/VAT
        //        if (cmbGstCstVat.Value != null)
        //        {
