    ...
    public partial class management_frmAddDocumentCandidate : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (DocyType[0, 0] != "n")
                {
                    clsdropdown.AddDataToDropDownList(DocyType, DTYpe);
                }
                string[,] Building1 = oDBEngine.GetFieldValue("tbl_master_building", "bui_id,bui_Name", null, 2, "bui_Name");
                if (Building1[0, 0] != "n")
                {
                    clsdropdown.AddDataToDropDownList(Building1, Building, true);
                }
            }
        }
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                popupScript = "<script language='javascript'>" + "alert('Successfully Uploaded');window.location.href='" + ViewState["previousPageUrl"] + "';</script>";
                ClientScript.RegisterStartupScript(GetType(), "PopupScript", popupScript);
            }
