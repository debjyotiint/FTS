...
{
    public partial class management_SettingsOptions_targetSetUpsubbroker : System.Web.UI.Page, ICallbackEventHandler
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                TxtBranchName.Attributes.Add("onkeyup", "ajax_showOptions(this,'SearchBySubBroker',event)");
        ...
        {
            //DataTable DtTable = oDBEngine.GetDataTable("tbl_master_branch b,tbl_targetschemes st,tbl_targetsetup t", "b.branch_description,st.tgt_descripition,convert(varchar(10),t.tgtset_applicabledate,113) as ApplicableDate,convert(varchar(10),isnull(t.tgtset_tilldate,''),113) as Tilldate ", " st.tgt_id=t.tgtset_scheme and t.tgtset_Entity=b.branch_internalId and b.branch_type='Sub Broker'");
            DataTable DtTable = oDBEngine.GetDataTable("tbl_master_branch b,tbl_targetschemes st,tbl_targetsetup t", "b.branch_description,st.tgt_descripition,convert(varchar(10),convert(datetime,t.tgtset_applicabledate,103),113) as ApplicableDate,convert(varchar(10),convert(datetime,isnull(t.tgtset_tilldate,''),103),113) as Tilldate ", " st.tgt_id=t.tgtset_scheme and t.tgtset_Entity=b.branch_internalId and b.branch_type='Sub Broker'");
