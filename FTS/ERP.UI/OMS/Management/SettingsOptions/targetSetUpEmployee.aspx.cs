...
{
    public partial class management_SettingsOptions_targetSetUpEmployee : System.Web.UI.Page, ICallbackEventHandler
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                //string[,] cmbData = oDBEngine.GetFieldValue("tbl_targetschemes", "tgt_id,tgt_descripition", " CAST(tgt_effectivedate AS datetime) < CAST(GETDATE() AS datetime) AND CAST(tgt_enddate AS datetime) > CAST(GETDATE() AS datetime)", 2, "tgt_descripition");
                ...
                string[,] cmbData = oDBEngine.GetFieldValue("tbl_targetschemes", "tgt_id,tgt_descripition", "  tgt_effectivedate < convert(datetime, GETDATE(),103) AND  tgt_enddate > convert(datetime, GETDATE(),103) and tgt_enddate is not null", 2, "tgt_descripition");
                if (cmbData[0, 0] != "n")
                {
                ...
                TxtBranchName.Attributes.Add("onkeyup", "ajax_showOptions(this,'SearchByEmployees',event)");
                //ImgStartDate.Attributes.Add("OnClick", "displayCalendar(TxtApplicableDate,'dd/mm/yyyy',TxtApplicableDate,true,null,'0',0)");
                //TxtApplicableDate.Attributes.Add("onfocus", "displayCalendar(TxtApplicableDate ,'dd/mm/yyyy',this,true,null,'0',0)");
        ...
        {
            //DataTable DtTable = oDBEngine.GetDataTable("tbl_master_contact contact,tbl_targetschemes st,tbl_targetsetup t", "(contact.cnt_firstName +' '+contact.cnt_middleName+' '+contact.cnt_lastName) as Name,st.tgt_descripition,convert(varchar(10),t.tgtset_applicabledate,113) as ApplicableDate,convert(varchar(10),isnull(t.tgtset_tilldate,''),113) as Tilldate ", " st.tgt_id=t.tgtset_scheme and t.tgtset_Entity=contact.cnt_internalId");

            DataTable DtTable = oDBEngine.GetDataTable("tbl_master_contact contact,tbl_targetschemes st,tbl_targetsetup t", "(contact.cnt_firstName +' '+contact.cnt_middleName+' '+contact.cnt_lastName) as Name,st.tgt_descripition,convert(varchar(20),convert(datetime,t.tgtset_applicabledate,103),113) as ApplicableDate,convert(varchar(20),convert(datetime,isnull(t.tgtset_tilldate,''),103),113) as Tilldate ", " st.tgt_id=t.tgtset_scheme and t.tgtset_Entity=contact.cnt_internalId");
