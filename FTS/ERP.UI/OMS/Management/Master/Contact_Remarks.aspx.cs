...
{
    public partial class management_master_Contact_Remarks : ERP.OMS.ViewState_class.VSPage
    {
        string[,] FieldName;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                lblHeadTitle.Text = Session["requesttype"].ToString() + " UDF";
            }
            ...
            //rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/master/Contact_Remarks.aspx");

            // bellow code added by debjyoti 
            //Reason: Rights of user set from parents 17-11-2016
            ...
            else if (Session["requesttype"].ToString() == "Relationship Partner")
            {
                Session["KeyVal1"] = "RP";
            }
            else if (Session["requesttype"].ToString() == "Sub Broker")
            {
                Session["KeyVal1"] = "Sb";
            }
            else if (Session["requesttype"].ToString() == "Franchisee")
            {
                Session["KeyVal1"] = "fr";
            }
            else if (Session["requesttype"].ToString() == "Data Vendor")
            {
                Session["KeyVal1"] = "DV";
            }
            else if (Session["requesttype"].ToString() == "OtherEntity")
            {
                Session["KeyVal1"] = "XC";
            }
            else if (Session["requesttype"].ToString().ToLower().Trim().Contains("lead"))
            {
                Session["KeyVal1"] = "Ld";
            }
            if (Session["Name"] != null)
            {
                try
                {
                    sqlCategory.SelectParameters["KeyVal1"].DefaultValue = Session["KeyVal1"].ToString();
                }
                catch (Exception ex)
                {
                    sqlCategory.SelectParameters["KeyVal1"].DefaultValue = "";
                }
                lblName.Text = Session["Name"].ToString();
            }
            if (Session["requesttype"].ToString() == "OtherEntity")
            {
                //txtClentUcc.Visible = false;
            ...
            //Bellow function will load all the control with res[ect to there module , Module Id pass by the session variable  session[keval1]
            // loadControls(); 
            loadTab();
                    ...
                    foreach (object ob in page.Controls)
                    {
                        if (ob is TextBox)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                //{
                                //    GridRemarks.JSProperties["cpErrorMsg"] = "Value cannot be greater than " + Convert.ToString(dr["cat_max_value"]);
                                //    retval = false;
                                //    continue;
                                //}
                            }
