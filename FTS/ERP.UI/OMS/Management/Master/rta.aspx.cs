...
{
    public partial class management_master_rta : ERP.OMS.ViewState_class.VSPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (HttpContext.Current.Session["EntryProfileType"] != null)
            {
                ...
                {
                    rtaSource.ConnectionString = ConfigurationSettings.AppSettings["DBReadOnlyConnection"];
                    rtaAddress.ConnectionString = ConfigurationSettings.AppSettings["DBReadOnlyConnection"];
                    rtaPhone.ConnectionString = ConfigurationSettings.AppSettings["DBReadOnlyConnection"];
                    rtaEmail.ConnectionString = ConfigurationSettings.AppSettings["DBReadOnlyConnection"];

                }
                else
                {
                    rtaSource.ConnectionString = ConfigurationSettings.AppSettings["DBConnectionDefault"];
                    rtaAddress.ConnectionString = ConfigurationSettings.AppSettings["DBConnectionDefault"];
                    rtaPhone.ConnectionString = ConfigurationSettings.AppSettings["DBConnectionDefault"];
                    rtaEmail.ConnectionString = ConfigurationSettings.AppSettings["DBConnectionDefault"];
                }
            }
        ...
        {
            rtaGrid.Settings.ShowFilterRow = true;
        }
        ...
        protected void rtaGrid_HtmlRowCreated(object sender, ASPxGridViewTableRowEventArgs e)
        {
            if (e.RowType == GridViewRowType.Data)
            {
                int commandColumnIndex = -1;
                for (int i = 0; i < rtaGrid.Columns.Count; i++)
                    if (rtaGrid.Columns[i] is GridViewCommandColumn)
                    {
        ...
        protected void rtaGrid_HtmlEditFormCreated(object sender, ASPxGridViewEditFormEventArgs e)
        {
            if (!rtaGrid.IsNewRowEditing)
            {
                ASPxPageControl RT1 = rtaGrid.FindEditFormTemplateControl("pageControl") as ASPxPageControl;
        ...
        protected void rtaGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            if (e.Parameters == "s")
                rtaGrid.Settings.ShowFilterRow = true;
            ...
            if (e.Parameters == "All")
            {
                rtaGrid.FilterExpression = string.Empty;
            }
        }
        protected void PhoneGrid_RowValidating(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e)
        {
            ASPxPageControl tab = rtaGrid.FindEditFormTemplateControl("pageControl") as ASPxPageControl;
