...
namespace ERP.OMS.Management.Master
{
    public partial class SAC : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Master/SAC.aspx");
        ...
        protected void gridudfGroup_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            try
            {
                ...
                if (lengthIndex[0].ToString() == "SAVE_NEW")
                {
                    ...
                    int retData = pbl.InsertSAC(txtCode.Text.Trim(), txtDescription.Text.Trim(),txtGroupId.Text.Trim(), txtGroupName.Text.Trim(), "SAC");
                    if (retData==1)
                    {
                    ...
                    else if (retData == 999)
                    {
                        ...
                        gridudfGroup.JSProperties["cpMsg"] = "SAC Code already exists";
                    }
                }
        ...
        private void DataBinderSegmentSpecific()
        {
            ...
            SqlDataSource1.SelectCommand = "select Tax_ID,Service_Category_Code,Service_tax_Name,Group_ID,Group_Name from tbl_master_service_tax";
