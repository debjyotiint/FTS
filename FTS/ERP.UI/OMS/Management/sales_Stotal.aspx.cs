...
{
    public partial class management_sales_Stotal : System.Web.UI.Page
    {
        ...
        public void BindGrid()
        {
            ...
            dt.Columns.Add(col6);
            dt.Columns.Add(col7);
            dt1 = oDBEngine.GetDataTable("tbl_trans_SExpenditure", "sum(expnd_travExpence) as ConExp,sum(expnd_travTravExpence) as TravExp,sum(expnd_LodgingExpnseAmount) as LodgingExp,sum(expnd_FoodExpnseAmount) as FoodingExp,sum(expnd_BPExpAmount) as BPExp,sum(expnd_otherExpnseAmount) as OtherExp", " expnd_empId='" + Session["SalesID"].ToString() + "'");
            if (dt1.Rows.Count != 0)
            {
