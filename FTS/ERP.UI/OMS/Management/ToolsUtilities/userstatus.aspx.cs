...
namespace ERP.OMS.Management.ToolsUtilities
{
    public partial class management_ToolsUtilities_userstatus : System.Web.UI.Page
    {
        ...
        public void ShowActiveList()
        {
            ...
            dtActivelist_New.Columns.Add(new DataColumn("seg_name", typeof(String))); //3
            dtActivelist_New.Columns.Add(new DataColumn("user_activity", typeof(String))); //4
            dtActivelist_New.Columns.Add(new DataColumn("Since", typeof(String))); //5
            dtActivelist_New.Columns.Add(new DataColumn("Duration", typeof(String))); //6
            dtActivelist_New.Columns.Add(new DataColumn("user_lastIP", typeof(String))); //7
            dtActivelist = oDbEngine.GetDataTable("tbl_master_user,tbl_master_contact,tbl_master_segment", "user_id,user_loginid,isnull(rtrim(cnt_firstname),'')+' '+isnull(rtrim(cnt_middlename),'')+isnull(rtrim(cnt_lastname),'') as UserName,seg_name,user_activity,CONVERT(CHAR(19), cast(tbl_master_user.LastModifyDate as datetime), 0) as Since,DATEDIFF(minute, cast(tbl_master_user.LastModifyDate as datetime),getdate()) as Duration,user_lastIP", " cnt_internalid=user_contactid and user_status=1 and user_id not in(" + Session["userid"].ToString() + ") and seg_id=user_lastsegement", " cnt_firstname");
            if (dtActivelist.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtActivelist.Rows.Count; i++)
                {
                    ...
                    string ParntID = null;
                    if (dtActivelist.Rows[i][4] == DBNull.Value)
                        ParntID = "0";
                    else
                        ParntID = dtActivelist.Rows[i][4].ToString();
                    ...
                    drActivity[4] = FinalActivityName;
                    drActivity[5] = dtActivelist.Rows[i][5].ToString();
                    drActivity[6] = dtActivelist.Rows[i][6].ToString();
                    drActivity[7] = dtActivelist.Rows[i][7].ToString();
