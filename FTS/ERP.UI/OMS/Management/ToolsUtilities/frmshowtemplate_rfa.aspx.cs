...
{
    public partial class management_utilities_frmshowtemplate_rfa : System.Web.UI.Page
    {
        ...
        {
            string temp_id = Request.QueryString["tem_id"].ToString();
            string[,] recipient = oDBEngine.GetFieldValue("tbl_master_rfa", "rfa_target1", " rfa_id='" + temp_id + "'", 1);
            if (recipient[0, 0] != "n")
            {
                HREC.Value = recipient[0, 0];
            }
            string[,] msg1 = oDBEngine.GetFieldValue("tbl_master_rfa", "rfa_description", " rfa_id='" + temp_id + "'", 1);
            if (msg1[0, 0] != "n")
            {
