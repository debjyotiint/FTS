<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/OMS/MasterPage/ERP.Master"
    Inherits="ERP.OMS.Management.ToolsUtilities.management_utilities_frm_attendance_Lock_iframe" CodeBehind="frm_attendance_Lock_iframe.aspx.cs" %>
            ...
            <tr>
                <td colspan="2">
                    <asp:GridView ID="GrdAttendanceLock" runat="server" AllowPaging="True" AutoGenerateColumns="False"
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="Month" SortExpression="al_month">
                                ...
                                <ItemStyle CssClass="gridcellleft" />
                                <EditItemTemplate>
                                    <asp:DropDownList ID="cmbMonthE" runat="server" SelectedValue='<%# Eval("al_month") %>'
                                        Width="100%" CssClass="Ecoheadtxt" Enabled="false">
                                        <asp:ListItem Value="1">Jan</asp:ListItem>
                                        <asp:ListItem Value="2">Feb</asp:ListItem>
                                        <asp:ListItem Value="3">Mar</asp:ListItem>
                                        <asp:ListItem Value="4">Apr</asp:ListItem>
                            ...
                            <asp:TemplateField HeaderText="Year" SortExpression="al_year">
                                ...
                                <EditItemTemplate>
                                    <asp:DropDownList ID="cmbYearE" runat="server" SelectedValue='<%# Eval("al_year") %>'
                                        Width="100%" CssClass="Ecoheadtxt" DataSourceID="dataYear" DataTextField="yer"
                                        DataValueField="yer" Enabled="false">
                                    </asp:DropDownList>
                                </EditItemTemplate>
                            </asp:TemplateField>
                            <asp:TemplateField HeaderText="Lock" SortExpression="al_lock">
                                <ItemTemplate>
                                    <asp:Label ID="lblLock" runat="server" Text='<%# Eval("al_lockD") %>'></asp:Label>
                                </ItemTemplate>
                                <ItemStyle CssClass="gridcellleft" />
                                <EditItemTemplate>
                                    <asp:DropDownList ID="cmbLockE" runat="server" SelectedValue='<%# Eval("al_lock") %>'
                    ...
                    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="OverwriteChanges"
                        ...
                        InsertCommand="INSERT INTO [tbl_trans_AttendanceLock] ([al_company], [al_branch], [al_month], [al_year], [al_lock]) VALUES (@al_company, @al_branch, @al_month, @al_year, @al_lock)"
                        SelectCommand="SELECT [al_id], [al_company],(select ltrim(rtrim(cmp_name)) from tbl_master_company where cmp_internalid=al_company) as al_companyD, [al_branch],(select branch_description from tbl_master_branch where branch_id=al_branch) as al_branchD, [al_month],(case when al_month=1 then 'Jan' when al_month=2 then 'Feb' when al_month=3 then 'Mar' when al_month=4 then 'Apr' when al_month=5 then 'May' when al_month=6 then 'Jun' when al_month=7 then 'Jul' when al_month=8 then 'Aug' when al_month=9 then 'Sep' when al_month=10 then 'Oct' when al_month=11 then 'Nov' when al_month=12 then 'Dec' else '' end) as al_monthD, [al_year], [al_lock],(case when al_lock='Y' then 'Locked' else 'Open' end) as al_lockD FROM [tbl_trans_AttendanceLock] ORDER BY [al_month] DESC, [al_year] DESC"
                    ...
                    <asp:SqlDataSource ID="dataCompany" runat="server" ConnectionString="<%$ ConnectionStrings:crmConnectionString %>"
                        SelectCommand="SELECT [cmp_internalid], [cmp_Name] FROM [tbl_master_company] ORDER BY [cmp_Name]"></asp:SqlDataSource>
                    <asp:SqlDataSource ID="databranch" runat="server" ConnectionString="<%$ ConnectionStrings:crmConnectionString %>"
                        SelectCommand="SELECT [branch_id], [branch_description] FROM [tbl_master_branch] ORDER BY [branch_description]"></asp:SqlDataSource>
                    <asp:SqlDataSource ID="dataYear" runat="server" ConnectionString="<%$ ConnectionStrings:crmConnectionString %>"
