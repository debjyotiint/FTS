...
{
    public partial class management_activitymanagement_frm_SalesVisitActivityWithIframe : System.Web.UI.Page
    {

        BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
        BusinessLogicLayer.Converter objConverter = new BusinessLogicLayer.Converter();
        clsDropDownList clsdropdown = new clsDropDownList();
        private string[] hd1UserList1 = null;
        int pageindex = 0;
        int pagecount = 0;
        int pageSize;
        int rowcount = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                //txtAssigned.Attributes.Add("onblur", "JavaScript:SetAddLeadEnable();");
                //Page.ClientScript.RegisterStartupScript(GetType(), "Disable", "<script language='JavaScript'>MakeButtonDisable();</script>"); 
        ...
        public void BinddropDown()
        {
            ...
            if (userWork[0, 0] != "n")
            {
                //oDBEngine.AddDataToDropDownList(userWork, drpUserWork);
            }
        }
        ...
        protected void btnCreate_Click(object sender, EventArgs e)
        {
            ...
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightForCreate", "<script language='JavaScript'>height();</script>");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "CheckLeadProduct", "<script language='JavaScript'>alert('Please enter Lead and Product')</script>");
            }
            else
            {
                switch (ViewState["status"].ToString())
                {
                    case "CreateNew":
                        try
                        {
                            if (Session["lead"] != null && Session["Product"] != null)
                            {
                                ...
                                for (int rowa = 0; rowa < assignto.Length; rowa++)
                                {
                                    rowCount = oDBEngine.InsurtFieldValue("tbl_trans_Activies", "act_branchId,act_activityType,act_description,act_assignedBy,act_assignedTo,act_createDate,act_contactlead,act_scheduledDate,act_expectedDate,act_activityno,act_priority,act_scheduledTime,act_expectedTime,CreateDate,CreateUser", "'" + HttpContext.Current.Session["userbranchID"].ToString() + "','" + drpActType.SelectedItem.Value + "','" + txtDesc.Text + "','" + Session["userid"].ToString() + "','" + assignto[rowa].ToString() + "','" + oDBEngine.GetDate().ToShortDateString() + "','" + Session["Count"].ToString() + "','" + sStartDate1 + "','" + sEndDate + "','" + ActNo + "','" + drpPriority.SelectedItem.Value + "','" + sStartTime + "','" + sEndTime + "','" + oDBEngine.GetDate().ToString() + "','" + Session["userid"].ToString() + "'");//'" + drpUserWork.SelectedItem.Value + "'
                                    //string[,] actid;
                                    ActidForProduct = oDBEngine.GetFieldValue("tbl_trans_Activies", "act_id", "act_activityno='" + ActNo.ToString() + "' and act_assignedto='" + assignto[rowa].ToString() + "'", 1);

                                    //string a = hd1UserList.Value;
                                    if (rowCount != 0)
                                    {
                                        ...
                                        foreach (string chrk in temp)
                                        {
                                            ...
                                            {
                                                string ofpId = "";
                                                string[] ProductValues = Session["Product"].ToString().Split(':');
                                                oDBEngine.InsurtFieldValue("tbl_trans_offeredProduct", "ofp_actid,ofp_activityId, ofp_leadId, ofp_productTypeId, ofp_productId, ofp_probableAmount,CreateDate,CreateUser", "'" + ActidForProduct[0, 0].ToString() + "','" + ActNo + "','" + chrk1.GetValue(0).ToString() + "','" + ProductValues.GetValue(0).ToString() + "','" + ProductValues.GetValue(2).ToString() + "','" + ProductValues.GetValue(1).ToString() + "','" + oDBEngine.GetDate().ToString() + "','" + Session["userid"].ToString() + "'");
                            ...
                            {
                                Page.ClientScript.RegisterStartupScript(GetType(), "CheckLeadProduct", "<script language='JavaScript'>alert('Please Add Lead and Product')</script>");
                            }
                        }
                        catch
                        {
                    ...
                    case "Modify":
                        ...
                        column += ",act_assignedTo='TEST'";//" + drpUserWork.SelectedItem.Value + "
                ...
                Response.Redirect("frm_SalesVisitActivityWithIframe.aspx");
        ...
        protected void btnshowGenratedSaleVisit_Click(object sender, EventArgs e)
        {
            ...
            if (productcon == "")
            {
                ...
                else
                {
                    if (Lrd.Checked == true)
                    {
                        dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_lead ON tbl_master_address.add_cntId = tbl_master_lead.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_lead.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_lead.cnt_firstName, '') + ' ' + ISNULL(tbl_master_lead.cnt_middleName, '') + ' ' + ISNULL(tbl_master_lead.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar,tbl_trans_salesVisit.slv_nextvisitdatetime,100) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId,tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", " (tbl_trans_Activies.act_assignedTo IN (" + all_user + ")) and (tbl_trans_salesVisit.slv_nextvisitdatetime >='" + fdate + "') and (tbl_trans_salesVisit.slv_nextvisitdatetime <= '" + todate + "') and (tbl_master_address.add_activityId IS NOT NULL)");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_contact ON tbl_master_address.add_cntId = tbl_master_contact.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_contact.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_contact.cnt_firstName, '') + ' ' + ISNULL(tbl_master_contact.cnt_middleName, '') + ' ' + ISNULL(tbl_master_contact.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar,tbl_trans_salesVisit.slv_nextvisitdatetime,100) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId,tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", " (tbl_trans_Activies.act_assignedTo IN (" + all_user + ")) and (tbl_trans_salesVisit.slv_nextvisitdatetime >='" + fdate + "') and (tbl_trans_salesVisit.slv_nextvisitdatetime <= '" + todate + "') and (tbl_master_address.add_activityId IS NOT NULL)");
                    }
                }
            }
            else
            {
                ...
                else
                {
                    if (Lrd.Checked == true)
                    {
                        dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_lead ON tbl_master_address.add_cntId = tbl_master_lead.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_lead.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_lead.cnt_firstName, '') + ' ' + ISNULL(tbl_master_lead.cnt_middleName, '') + ' ' + ISNULL(tbl_master_lead.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar(17),tbl_trans_salesVisit.slv_nextvisitdatetime,113) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId, tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", " (tbl_trans_Activies.act_assignedTo IN (" + all_user + ")) and (CAST(tbl_trans_Activies.act_scheduledDate AS datetime) >= CONVERT(varchar(10),'" + fdate + "', 101)) and (CAST(tbl_trans_Activies.act_scheduledDate AS datetime) <= CONVERT(varchar(10),'" + todate + "', 101)) and (ofp_producttypeid='" + drpselectedproduct + "') and (tbl_master_address.add_activityId IS NOT NULL)");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_contact ON tbl_master_address.add_cntId = tbl_master_contact.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_contact.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_contact.cnt_firstName, '') + ' ' + ISNULL(tbl_master_contact.cnt_middleName, '') + ' ' + ISNULL(tbl_master_contact.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar(17),tbl_trans_salesVisit.slv_nextvisitdatetime,113) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId, tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", " (tbl_trans_Activies.act_assignedTo IN (" + all_user + ")) and (CAST(tbl_trans_Activies.act_scheduledDate AS datetime) >= CONVERT(varchar(10),'" + fdate + "', 101)) and (CAST(tbl_trans_Activies.act_scheduledDate AS datetime) <= CONVERT(varchar(10),'" + todate + "', 101)) and (ofp_producttypeid='" + drpselectedproduct + "') and (tbl_master_address.add_activityId IS NOT NULL)");
                    }
                }
            }
            ...
            pageSize = 5;
            pagecount = dt_main.Rows.Count / pageSize + 1;
            ...
            grdGenratedSalesVisit.PageIndex = pageindex;
            ...
            //if (pagecount == 1)
            //{
            //    grdGenratedSalesVisit.BottomPagerRow.Visible = true;
            ...
            txtLeadId.Value = "";
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightForSales", "<script language='JavaScript'>height();</script>");
            if (Radio1.Checked == true)
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "All", "<script language='JavaScript'>fun(this,true);</script>");
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "Selective", "<script language='JavaScript'>fun(this,false);</script>");
            }
        ...
        protected void btnReallocateSalesVisit_Click(object sender, EventArgs e)
        {
            ...
            frmAllot.Visible = false;
            Td1.Visible = false;
            Session["selectedbutton"] = "salesvisit";
            DataTable dt = new DataTable();
            dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_contact ON tbl_master_address.add_cntId = tbl_master_contact.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_contact.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_contact.cnt_firstName, '') + ' ' + ISNULL(tbl_master_contact.cnt_middleName, '') + ' ' + ISNULL(tbl_master_contact.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar(17),tbl_trans_salesVisit.slv_nextvisitdatetime,113) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId, tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", "1=2");
            grdGenratedSalesVisit.DataSource = dt;
            grdGenratedSalesVisit.DataBind();
            lblTotalRecord.Text = "";
            //Td1 for Sales visit
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightReallocate", "<script language='JavaScript'>height();</script>");
        ...
        protected void grdGenratedSalesVisit_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (ViewState["GSales"] != null)
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    {
                        string[,] CheckUser = oDBEngine.GetFieldValue("tbl_master_user", "user_id", "user_name='" + e.Row.Cells[8].Text.ToString() + "'", 1);
                        if (CheckUser[0, 0] != Session["UserId"].ToString())
                        {
                            chk.Checked = true;
                            chk.Enabled = false;
                        }
        ...
        protected void btnMainCancel_Click(object sender, EventArgs e)
        {
            ...
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightMainCancel", "<script language='JavaScript'>height();</script>");
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            btnMainCancel.Visible = false;
            txtAssigned.Text = "";
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightCancel", "<script language='JavaScript'>height();</script>");
        ...
        protected void btnCancelGenratedSalesVisit_Click(object sender, EventArgs e)
        {
            ...
            userInfo1.Visible = true;
            btnMainCancel.Visible = false;
            Page.ClientScript.RegisterStartupScript(GetType(), "HeightCSales", "<script language='JavaScript'>height();</script>");
        ...
        protected void btnGenratedSales_Click(object sender, EventArgs e)
        {
            ...
            frmAllot.Visible = false;
            Td1.Visible = false;
            Session["selectedbutton"] = "sales";
            DataTable dt = new DataTable();
            dt = oDBEngine.GetDataTable("tbl_trans_Activies INNER JOIN tbl_trans_salesVisit INNER JOIN tbl_trans_offeredProduct ON tbl_trans_salesVisit.slv_leadcotactId = tbl_trans_offeredProduct.ofp_leadId ON tbl_trans_Activies.act_id = tbl_trans_salesVisit.slv_activityId AND tbl_trans_Activies.act_activityNo = tbl_trans_offeredProduct.ofp_activityId INNER JOIN tbl_master_address INNER JOIN tbl_master_contact ON tbl_master_address.add_cntId = tbl_master_contact.cnt_internalId ON tbl_trans_salesVisit.slv_leadcotactId = tbl_master_contact.cnt_internalId", "tbl_trans_salesvisit.slv_leadcotactid, tbl_trans_offeredProduct.ofp_productTypeId AS Product, ISNULL(tbl_master_contact.cnt_firstName, '') + ' ' + ISNULL(tbl_master_contact.cnt_middleName, '') + ' ' + ISNULL(tbl_master_contact.cnt_lastName, '') AS Name, ISNULL(tbl_master_address.add_landMark, '') + '[' + ISNULL(tbl_master_address.add_address1, '') + ' , ' + ISNULL(tbl_master_address.add_address2, '') + ' , ' + ISNULL(tbl_master_address.add_address3, '') + '] ' +  ISNULL(tbl_master_address.add_pin, '') AS Address, convert(varchar(17),tbl_trans_salesVisit.slv_nextvisitdatetime,113) as slv_nextvisitdatetime, tbl_trans_salesVisit.slv_leadcotactId, tbl_trans_Activies.act_assignedBy as Assignedby,tbl_trans_Activies.act_assignedto as act_assignedTo, tbl_trans_offeredProduct.ofp_Id as ProductId", "1=2");
            grdGenratedSalesVisit.DataSource = dt;
            grdGenratedSalesVisit.DataBind();
            lblTotalRecord.Text = "";
            Page.ClientScript.RegisterStartupScript(GetType(), "Height1", "<script language='JavaScript'>height();</script>");
        ...
        {
            Page.ClientScript.RegisterStartupScript(GetType(), "CallExport", "<script language='JavaScript'>CallingExport();</script>");
        }
