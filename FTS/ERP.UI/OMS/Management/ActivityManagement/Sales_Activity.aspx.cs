    ...
    public partial class Sales_Activity : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
        ...
        public void BindSalesActivity()
        {
            ...
            if (ds != null)
            {
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //txtStartDate.Date = Convert.ToDateTime(Convert.ToString(dtActivity.Rows[0]["act_scheduledDate"]) + " " + Convert.ToString(dtActivity.Rows[0]["act_scheduledTime"]));
                    //txtEndDate.Date =  Convert.ToDateTime(Convert.ToString(dtActivity.Rows[0]["act_expectedDate"]) + " " + Convert.ToString(dtActivity.Rows[0]["act_expectedTime"])) ;
                    //drpPriority.SelectedValue = Convert.ToString(dtActivity.Rows[0]["act_priority"]);
                    ...
                    //if(type=="1")
                    //{ BindClientByIndustryId(lbClientAvailable, "0", hdnEditClientType.Value); }
                    //else { BindClientByIndustryId(lbClientAvailable, hdnEditIndustry.Value, hdnEditClientType.Value); }
        ...
        public void SelectedExistProductClassByVal(string pitems, bool IsSelected, object sender)
        {
            ...
            for (int i = 0; i < list_counter; i++)
            {
                ...
                // else { finalList.Items[i].Selected = false; }
            }
        ...
        public void bindAllTasks()
        {
            ...
            //Reason: select titlte  value must be 0
            //taskList.Items.Insert(0, "Select Title");
            taskList.Items.Insert(0, new ListItem("Select Task", "0"));


            //taskDescList.DataSource = DT;
            //taskDescList.DataValueField = "task_id";
            //taskDescList.DataTextField = "task_description";
            //taskDescList.DataBind();
        }
