...
{
    public partial class management_Frm_SendEmailWithTemplate : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
    {
        string data;
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            ...
            if (DT.Rows.Count > 0)
            {
                for (int i = 0; i < DT.Rows.Count; i++)
                {
                    ...
                    //        da.SelectCommand.Parameters.AddWithValue("@CompanyID", dtCFG.Rows[0][0].ToString());
                    ...
                    dsCnt = oManagement_BL.Fetch_EmailTemplateReservedWord(Convert.ToString(cmbType.SelectedItem.Value), Convert.ToString(DT.Rows[i]["ID"]),
                        Convert.ToString(dtCFG.Rows[0][0]));
                    dtCnt = dsCnt.Tables[0];

                    string TempContent = string.Empty;
                    TempContent = textEditor.Text;

                    DataTable dtUser = oDBEngine.GetDataTable(" TBL_MASTER_CONTACT  ", "RTRIM(LTRIM(ISNULL(CNT_FIRSTNAME,' ')))+ ' ' + LTRIM(RTRIM(ISNULL(CNT_MIDDLENAME,' '))) +' '+ LTRIM(RTRIM(ISNULL(CNT_LASTNAME,' ')))  AS USER_NAME ,LTRIM(RTRIM(ISNULL(CNT_UCC,CNT_SHORTNAME))) AS USER_CODE,(SELECT  DEG_DESIGNATION  FROM TBL_MASTER_DESIGNATION  WHERE DEG_ID IN(SELECT EMP_DESIGNATION FROM  TBL_TRANS_EMPLOYEECTC WHERE EMP_CNTID=CNT_INTERNALID)) AS USER_DESIGNATION,(SELECT  CMP_NAME  FROM TBL_MASTER_COMPANY  WHERE CMP_ID IN(SELECT EMP_ORGANIZATION FROM  TBL_TRANS_EMPLOYEECTC WHERE EMP_CNTID=CNT_INTERNALID)) AS USER_COMPANY,(SELECT BRANCH_DESCRIPTION FROM TBL_MASTER_BRANCH WHERE BRANCH_ID=CNT_BRANCHID) AS  USER_BRANCH ", "CNT_INTERNALID  IN ( SELECT USER_CONTACTID FROM TBL_MASTER_USER WHERE USER_ID='" + Session["userid"].ToString() + "' ) ");
                    string[] strRec = { "#USER_NAME#", "#USER_CODE#", "#USER_DESIGNATION#", "#USER_COMPANY#", "#USER_BRANCH#" };
                    string[] strRecData = { "USER_NAME", "USER_CODE", "USER_DESIGNATION", "USER_COMPANY", "USER_BRANCH" };
                    for (int p = 0; p < strRec.Length; p++)
                    {
                        if (TempContent.Contains(strRec[p].ToString()) == true)
                        {
                            TempContent = TempContent.ToString().Replace(strRec[p].ToString(), dtUser.Rows[0][strRecData[p].ToString()].ToString());

                        }
                    }


                    string[] strArray = {"##FirstName##", "##MiddleName##", "##LastName##", "##ClientID##", 
                         "##Addres1##","##Addres2##", "##Addres3##", "##City##", "##State##", "##Country##", 
                         "##Pin##", "##ISDCode##", "##STDCode##", "##(TelephoneNumber##", "##MobNumber##", 
                         "##DateOfBirth##", "##ClientAgrementDate##", "##PANNumber##", "##CurrentDate##", 
                         "##TradingCode##"};

                    string[] strData = {"FirstName", "MiddleName", "LastName", "ClientID", 
                         "Addres1","Addres2", "Addres3", "City", "State", "Country", 
                         "Pin", "ISDCode", "STDCode", "#(TelephoneNumber", "MobNumber", 
                         "DateOfBirth", "ClientAgrementDate", "PANNumber", "CurrentDate", 
                         "TradingCode" };
                    ...
                    {
                        if (TempContent.Contains(strArray[s].ToString()) == true)
                        {
                            if (dtCnt.Rows[0][strData[s].ToString()].ToString() == "")
                            {
                                TempInd = "N";
                            }
                            else
                            {
                                TempContent = TempContent.ToString().Replace(strArray[s].ToString(), dtCnt.Rows[0][strData[s].ToString()].ToString());
                    ...
                    if (cmbType.SelectedItem.Value.ToString() == "CL" || cmbType.SelectedItem.Value.ToString() == "EM")
                    {
                        ...
                        TempContent = TempContent.ToString().Replace("#ClientID#", dtCnt.Rows[0]["ClientID"].ToString());
                    ...
                    else if (cmbType.SelectedItem.Value.ToString() == "CD")
                    {
                        ...
                        TempContent = TempContent.ToString().Replace("#PANNumber#", dtCnt.Rows[0]["PANNumber"].ToString());
                        TempContent = TempContent.ToString().Replace("#CurrentDate#", dtCnt.Rows[0]["CurrentDate"].ToString());
                        TempContent = TempContent.ToString().Replace("#TradingCode#", dtCnt.Rows[0]["TradingCode"].ToString());
                    ...
                    if (TempInd != "N")
                    {
                        ...
                        long result = 0;
                        oManagement_BL.InsertTransEmail(Convert.ToString(dtCFG.Rows[0][1]), Convert.ToString(TxtSubject.Text), Convert.ToString(TempContent),
                            "N", 1, Convert.ToInt32(Session["userid"]), "N", Convert.ToString(dtCFG.Rows[0][0]), "CRM", out result);
                        InternalID = Convert.ToString(result.ToString());
                        //  ###########---recipients-----------------                   

                        string fValues3 = "'" + InternalID + "','" + DT.Rows[i]["ID"].ToString() + "','" + DT.Rows[i]["EmailID"].ToString() + "','TO','" + oDBEngine.GetDate().ToString() + "','" + "P" + "'";
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "AlessrtF", "alert('Mandatory variable not found!...')", true);
                    }
