...
namespace ERP.OMS.Reports
{
    ...
    public partial class management_journalPopupReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                tDate.Value = Convert.ToDateTime(fDate);
        ...
        public void BindGridForEdit()
        {
            ...
            if (dtReport.Rows.Count > 0)
            {
                ...
                string gridForVoucherID = dtReport.Rows[0][1].ToString().Substring(0, 1);
                string gridForVoucher = dtReport.Rows[0][1].ToString().Substring(0, 2);
                if (gridForVoucher == "YG" || gridForVoucher == "YF")
                {
                    ViewState["gridForVoucher"] = gridForVoucher;
                    tDate.ClientEnabled = false;
                }
                else if (gridForVoucherID == "Y" || gridForVoucherID == "X" || gridForVoucherID == "Z")
                {
            ...
            if (Request.QueryString["id"] != null)
            {
                if (ViewState["gridForVoucher"] != null)
                {
        ...
        protected void grdAdd_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                if (ViewState["gridForVoucher"] != null)
                {
                    if (ViewState["gridForVoucher"].ToString() == "YF")
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string Voucher = Request.QueryString["id"].ToString().Substring(0, 2);
                if (Voucher == "YF")
                {
                    if (tempsettlementtype != "")
                    {
                    ...
                    SqlConnection con = new SqlConnection(conn);
                    con.Open();
                    SqlCommand com = new SqlCommand("xmlJournalVoucherUpdate_InterSegment", con);
                    com.CommandType = CommandType.StoredProcedure;
                    com.Parameters.AddWithValue("@journalData", tabledata);
                    com.Parameters.AddWithValue("@createuser", Session["userid"].ToString());
                    com.Parameters.AddWithValue("@finyear", Session["LastFinYear"].ToString());
                    ...
                    {
                        Page.ClientScript.RegisterStartupScript(GetType(), "Script", "<script language='javascript'>alert('No Record Will be Delete Or Modify')</script>");
                    }
                    con.Close();
                }
                else
