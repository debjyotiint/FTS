...
namespace OpeningBusinessLogic.Customerconsolidate
{

    public class Consolidatecustomer
    {
        ...
        public int InsertReplacementDetails(string modelXML, string Action,string Amount,string type,string CusID,int ModId)
        {
        ...
        public DataTable GetAgents(string Action, string brnchlist)
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("Proc_Opening_CustomerConsolidate");
            proc.AddPara("@Action", Action);
            proc.AddPara("@branch", brnchlist);
            dt = proc.GetTable();
            return dt;
        }
    ...
    public class ConsolidatedCustomerClass
    {
