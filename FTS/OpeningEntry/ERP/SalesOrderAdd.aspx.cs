...
namespace  OpeningEntry.ERP
{
    ...
    public partial class SalesOrderAdd : System.Web.UI.Page
    {
        ...
        //protected void Page_Init(object sender, EventArgs e)//Added on 06-03-2017
        //{
        ...
        public void GetAllDropDownDetailForSalesOrder(string UserBranch)
        {
            ...
            ///Sudip Pal
            ///Financial Year
            // DataTable Schemadt = objSlaesActivitiesBL1.GetNumberingSchema(strCompanyID, strBranchID, FinYear, "9", "Y");


            //Opening Entry
            DataTable Schemadt = objSlaesActivitiesBL.GetNumberingSchemaOpening(strCompanyID, strBranchID, FinYear, "9", "Y");
            ...
            if (IsDeleteFrom != "D")
            {
                try
                {
                   ...
                   .Select(g => g.Key);
                    ...
                    if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" || validate == "transporteMandatory" || validate == "duplicateProduct" || validate == "transporteMandatory")
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //}
                            string strFinYear = GetFinancialYearCheckAccordingDaterange(Convert.ToDateTime(dt_PLSales.Value)); 
                            int id = (ModifySalesOrder(MainOrderID, strSchemeType, UniqueQuotation, strQuoteDate, strQuoteExpiry, strCustomer, strContactName,
                                              Reference, strBranch, strAgents, strCurrency, strRate, strTaxType, strTaxCode, tempSalesOrderdt, TaxDetailTable, ActionType, OANumber, OADate, "0", QuotationDate, QuoComponent, tempWarehousedt, tempBillAddress, tempTaxDetailsdt, approveStatus, strFinYear));
                            if (id <= 0)
                            {
                                if (id == -1)
                                {
        ...
        public int ModifySalesOrder(string OrderID, string strSchemeType, string strOrderNo, string strOrderDate, string strOrderExpiry, string strCustomer, string strContactName,
                                    ...
                                    DataTable TaxDetailTable, string ActionType, string OANumber, string OADate, 
            string QuotationNumber, string QuotationDate, string QuotationIdList, DataTable Warehousedt, DataTable BillAddressdt,
            DataTable QuotationTaxdt, string approveStatus,string FinYear)
        {
            try
            {
        ...
        protected void ComponentPanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (WhichCall == "save")
            {
                ...
                else if (AddressStatus == "2") // Copy Billing to Shipping Address
                {
                    ...
                    string address3 = txtAddress3.Text;
                    string landmark = txtlandmark.Text;
                    string CountryTrim = Convert.ToString(CmbCountry.Value).TrimStart(',');
                    int country = Convert.ToInt32(CountryTrim);
                    string StateTrim = Convert.ToString(CmbState.Value).TrimStart(',');
                    int State = Convert.ToInt32(StateTrim);
                    string CityTrim = Convert.ToString(CmbCity.Value).TrimStart(',');
                    int city = Convert.ToInt32(CityTrim);
                    string areaTrim = Convert.ToString(CmbArea.Value).TrimStart(',');
                    int area = Convert.ToInt32(areaTrim);
                    string pinTrim = Convert.ToString(CmbPin.Value).TrimStart(',');
        ...
        protected void BindLookUp(String customer, string OrderDate)
        {
            ...
            QuotationTable = GetQuotationOnSalesOrder(customer, OrderDate, status);
            lookup_quotation.GridView.Selection.CancelSelection();
            lookup_quotation.DataSource = QuotationTable;
            lookup_quotation.DataBind();
        ...
        public DataTable GetQuotationOnSalesOrder(string customer, string OrderDate, string Status)
        {
            ...
            //proc.AddVarcharPara("@userbranchlist", 1000, Convert.ToString(context.Session["userbranchID"]));
        ...
        public void GetFinancialYearstartandEnddate()
        {
            ...
            if (Session["LastFinYear"] != null)
            {
                ...
                if (dtsale.Rows.Count > 0)
                {
                    ...
                    dt_PLSales.MaxDate = Convert.ToDateTime(Convert.ToString(dtsale.Rows[0]["FinYear_StartDate"])).AddDays(-1);
